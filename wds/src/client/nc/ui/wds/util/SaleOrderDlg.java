...
import nc.ui.wds.w80020204.MyClientUI;
	...
	public void onOk() {
		if (getbillListPanel().getHeadBillModel().getRowCount() > 1) {
			...
			if (retBillVos != null && retBillVos.length > 1) {
				...
				((MyClientUI) m_parent)
						.showWarningMessage("请选择至少两条数据进行合并订单操作!");
				return;
			}
	...
	public void loadHeadData() {
		try {
			...
			StringBuffer sWhere = new StringBuffer(
					" dr = 0 and fstatus =2 and boutendflag ='N' and daudittime is not null and VDEF5 ='3' and VDEF6 is null ");
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (a != null && a.length > 0 && a[0] != null) {
						...
						ArrayList bodyList = (ArrayList) IUAPQueryBS
								.executeQuery(
										"select csaleid,crowno ,cinvbasdocid ,nnumber ,npacknumber,corder_bid,cunitid  from so_saleorder_b where csaleid='"
												+ a[0].toString() + "'",
						...
						if (null != a[9] && !"".equals(a[9])) {
							salehvo
									.setAttributeValue("daudittime", a[9]);
						}
