...
public class W8004040204Impl implements Iw8004040204 {
		...
		if (null != itemList && itemList.size() > 0) {
			...
			for (int i = 0; i < itemList.size(); i++) {
				...
				if (i == itemList.size() - 1) {
					...
					// 判断结果集是否为空
					if (null != generaltList && generaltList.size() > 0) {
						StockInvOnHandVO item = (StockInvOnHandVO) generaltList
								.get(0);
	...
	public String autoPickAction(String pk_user, TbOutgeneralBVO[] generalb,
		...
		// TODO Auto-generated method stub
		if(generalb == null || generalb.length == 0)
			return null;
		...
		for (int i = 0; i < generalb.length; i++) {
			if (null == generalb[i])
				continue;
			if (PuPubVO.getString_TrimZeroLenAsNull(generalb[i].getCinventoryid())==null){
			...
			double asum = 0;// 应发数量
			// 获取应出辅数量
			asum = PuPubVO.getUFDouble_NullAsZero(generalb[i].getNshouldoutassistnum()).toDouble();

			wareVO = new StockInvOnHandVO[list.size()];
			wareVO = (StockInvOnHandVO[]) list.toArray(wareVO);
			int len = wareVO.length;
			StockInvOnHandVO warevo = null;
			for (int j = 0; j < len; j++) {
				warevo = wareVO[j];
