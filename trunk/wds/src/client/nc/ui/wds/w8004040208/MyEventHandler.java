						...
						if (null != list && list.size() > 0) {
							...
							if (null != header) {
								...
								if (null != list && list.size() > 0) {
									...
									iw04.whs_processAction("CANCELSIGN",
											"DELETE", "4I", _getDate()
													.toString(), billvo,
											generalh);
			...
			if (retu == 1) {
				...
				if (null != result) {
					...
					// 必须确认后的才能签字
						...
						iw04.whs_processAction("SAVEPICKSIGN", null, "4I",
	...
	protected void onzdqh() throws Exception {
		...
		if (results == 1) {
			...
			if (null != generalbVO && generalbVO.length > 0) {
				...
				String msg = iw04.autoPickAction(ClientEnvironment
				...
				if (null != msg) {
					...
					for (int i = 0; i < generalbVO.length; i++) {
						...
						// 调用接口查询缓存表中的实出主辅数量
						Object[] a = iw04.getNoutNum(generalbVO[i]
		...
		if (null != list && list.size() > 0) {
			...
			if (null != outhvo) {
				...
				sWhere = " dr = 0 and general_pk = '" + tmphvo.getGeneral_pk()
						+ "'";
				list = (ArrayList) iuap.retrieveByClause(TbOutgeneralBVO.class,
						sWhere);
				if (null != list && list.size() > 0) {
					...
					for (int i = 0; i < tmpbvo.length; i++) {
						...
						ArrayList batchList = (ArrayList) iuap.executeQuery(
						...
						String pk_invmandoc = "select pk_invmandoc from bd_invmandoc  where pk_invbasdoc='"
								...
								+ outhvo.getComp()
								+ "' and dr=0 ";
						ArrayList tmpList = (ArrayList) iuap.executeQuery(
