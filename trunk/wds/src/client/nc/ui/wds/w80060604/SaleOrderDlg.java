...
public class SaleOrderDlg extends BillSourceDLG {
	...
	public SaleOrderDlg(Container parent) {
		super(null, null, null, null, "1=1", "0604", null, null, null, parent);
		init();
	}
	...
	public void loadHeadData() {
		try {
			...
			ArrayList list = (ArrayList) IUAPQueryBS
					.executeQuery(
							"select csaleid,vreceiptcode ,ccustomerid ,creceiptcustomerid ,vreceiveaddress,cemployeeid,cbiztype,vnote ,dbilldate,dapprovedate from so_sale where "
			...
			if (list != null && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					...
					if (a != null && a.length > 0 && a[0] != null) {
						...
						String sql = "select count(csaleid) from tb_fydnew where billtype = 1 and dr = 0 and csaleid ='"
								+ a[0].toString() + "'";
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (a != null && a.length > 0 && a[0] != null) {
						...
						ArrayList bodyList = (ArrayList) IUAPQueryBS
								.executeQuery(
										"select csaleid,crowno ,cinvbasdocid ,nnumber ,npacknumber,corder_bid  from so_saleorder_b where csaleid='"
												+ a[0].toString() + "'",
