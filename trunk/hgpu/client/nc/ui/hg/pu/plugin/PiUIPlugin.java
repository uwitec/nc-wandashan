	...
	public void beforeButtonClicked(ButtonObject bo, SCMUIContext ctx)
		...
		if (bo.getCode().equals(IButtonConstInv.BTN_SAVE)) {
			...
			validntaxrate();
			 ...
			 Object co =ctx.getBillCardPanel().getHeadItem("vdef20").getValueObject();
			 int row =ctx.getBillListPanel().getHeadTable().getSelectedRow();
			 if(row >-1){
				  co =ctx.getBillListPanel().getHeadBillModel().getValueAt(row, "vdef20");
			 }
			 if(PuPubVO.getString_TrimZeroLenAsNull(co)!=null ){
