...
import nc.ui.ic.pub.query.SCMDefaultFilterEditor;
  ...
  protected class ButtonManager218 extends nc.ui.ic.pub.bill.GeneralButtonManager {
	...
	public ButtonManager218(GeneralBillClientUI clientUI) throws BusinessException {
	  ...
	  public void onButtonClicked(nc.ui.pub.ButtonObject bo) {
        ...
        //按钮事件处理
         try{
    	  if(false==nc.ui.ic.pub.tools.GenMethod.isProductEnabled(getEnvironment().getCorpID(),ProductCode.PROD_TO)){
    		  showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("4008busi","UPP4008busi-000131")/*@res "内部交易模块没有安装"*/);
    		  return ;
    	  }
...
//    	added by lirr 2009-06-17 判断销售模块是否启用
    	  if(false==nc.ui.ic.pub.tools.GenMethod.isProductEnabled(getEnvironment().getCorpID(),ProductCode.PROD_SO)){
    		  showHintMessage("销售模块没有安装");
    		  return ;
    	  }
        onAddToTran4331();
      }
	  ...
	  /**
	   * add by zhw 2011-01-24   校验内销实拨  实发数量=应发数量
	   */
	  public boolean onSave() {
		  String cbiztype =PuPubVO.getString_TrimZeroLenAsNull(getBillCardPanel().getHeadItem("cbiztype").getValueObject());
		  int rowCount = getBillCardPanel().getBillModel().getRowCount();
		  UFDouble shouldout = UFDouble.ZERO_DBL;
		  UFDouble out = UFDouble.ZERO_DBL;
		  if(cbiztype.equalsIgnoreCase(HgPubConst.IC_CBIZTYPE)){
			  for(int i=0;i<rowCount;i++){
				  shouldout=PuPubVO.getUFDouble_NullAsZero(getBillCardPanel().getBodyValueAt(i,"nshouldoutnum"));//应发数量
				  out=PuPubVO.getUFDouble_NullAsZero(getBillCardPanel().getBodyValueAt(i,"noutnum"));//实发数量
				  if(!shouldout.equals(out)){
					  showErrorMessage("第"+(i+1)+"行应发数量与实发数量不相等");
					  return  false;
				  }
			  }
		  }
	  ...
	  private void onAddToOrder(AggregatedValueObject[] vos) {
	    ...
	    try {
	      ...
	      // modified by liuzy 2009-8-26 上午10:54:38 连接数问题，GeneralBillClientUI.transBillData有同样的处理
//	      GeneralBillUICtl.procOrdEndAfterRefAdd((GeneralBillItemVO[])SmartVOUtilExt.getBodyVOs(vos), getBillCardPanel(), getBillType());
    ...
    private void onAddToTran4331() {
      ...
      try {
        ...
        // modified by liuzy 2009-9-1 下午07:52:13 连接数问题，GeneralClientUI.transBillDataDeal中会处理
//        GeneralBillUICtl.procOrdEndAfterRefAdd((GeneralBillItemVO[])SmartVOUtilExt.getBodyVOs(vos), getBillCardPanel(), getBillType());
