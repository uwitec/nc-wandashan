	...
	public void onBatchAction(String sAction) {
		...
		try {
			...
			//  modify by zhw  校验实际出库数量不能为零
				GeneralBillItemVO[] items = (GeneralBillItemVO[])voaBills[i].getChildrenVO();
				if(getInOutFlag()<0){
					for(GeneralBillItemVO item:items){
						if(UFDouble.ZERO_DBL.compareTo(PuPubVO.getUFDouble_NullAsZero(item.getNoutnum()))==0){
							showErrorMessage("单据"+voaBills[i].getVBillCode()+",第"+item.getCrowno()+"行实际出库数量为零,不能审批");
							return;	
						}
					}
				}
...
//    二次开发扩展
	...
	public String checkBillForAudit(GeneralBillVO vobill) {
		...
		if (vobill.getHeaderVO().getFbillflag() != null
			...
			return "单据[" + vobill.getHeaderVO().getVbillcode()
					+ "]不是自由状态，不能签字！";
		return null;
	}
		...
		try {
			...
			if (saParamValue != null
				...
				if (saParamValue[9] != null
						&& "仓库".equalsIgnoreCase(saParamValue[9].trim()))
					m_isWhInvRef = true;
				else
					m_isWhInvRef = false;
...
/*		if (null != getBillTypeCode() && ("4D".equals(getBillTypeCode()) || "4401".equals(getBillTypeCode()))){
	...
	protected void showTime(long lStartTime, String sTaskHint) {
		...
		SCMEnv.out("执行<" + sTaskHint + ">消耗的时间为：" + (lTime / 60000) + "分"
...
//						  oldvalue);
		...
		if (GeneralBillVO.getBillInOutFlag(getBillType()) == InOutFlag.OUT) {
			//liuys add for 鹤岗矿业  解决批次档案参照勾选"根据可用量过滤",voLot.getOnhandnumType()=5,让其不勾选在途
			if (voLot.getOnhandnumType() != null
					&& (voLot.getOnhandnumType().intValue() == 0 || voLot.getOnhandnumType().intValue() == 5)) {
