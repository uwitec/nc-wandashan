...
public class GeneralButtonManager implements IButtonManager, BillActionListener {
	...
	public void onButtonClicked(ButtonObject bo) {
		...
		if (getClientUI().m_bNeedBillRef
				&& getClientUI().isNeedBillRefWithBillType()) {
			if (bo.getParent() == getButtonTree().getButton(
				...
				// "+getClientUI().getBillTypeCode());
				// SCMEnv.out("刘家清测试跟踪，重新设定的按钮初始化菜单时的按钮: "+bo.getCode() +
				// ":"+bo.getName()+":"+bo.getTag());
		...
		if (bo == getButtonTree().getButton(ICButtonConst.BTN_LINE_ADD)) {
			...
			onFirst();
				...
				ICButtonConst.BTN_BROWSE_PREVIOUS))
			onPrevious();
		else if (bo == getButtonTree().getButton(ICButtonConst.BTN_BROWSE_NEXT))
			onNext();
		else if (bo == getButtonTree().getButton(
				ICButtonConst.BTN_BROWSE_BOTTOM))
			onLast();
		...
		// 由应发（收）自动填写实发（收）
			...
			onPickAuto();
			// modify by zhw  2011-04-19  带供应商到表体
			// 自动捡货完成后 将供应商带到出库单表体上 add by zhw 2011-0304
			try {
				getInfoByBatchCode();
			} catch (Exception e) {
//				 TODO Auto-generated catch block
				e.printStackTrace();
				showHintMessage("未获取到供应商");
//				showWarningMessage("asdfsd");
			}
		}
		else if (bo == getButtonTree().getButton(
		...
		// 单据导出
			...
			onAddNewRowNo();
		} else if (bo == getButtonTree().getButton(ICButtonConst.BTN_CARD_EDIT)) {
			onLineCardEdit();
		} else if (bo == getButtonTree()
				.getButton(ICButtonConst.BTN_BATCH_EDIT)) {
			onLineBatchEdit();
		} else if (bo == getButtonTree()
				.getButton(ICButtonConst.BTN_DMStateQry)) {
			onDMStateQuery();
		} else if (getM_funcExtend() != null) {
			// 支持功能扩展
			if (BillMode.Card == getM_iCurPanel())
				getM_funcExtend().doAction(bo, getClientUI(),
	...
	protected void onPasteLineToEnd() {
		// TODO 自动生成方法存根
		if (getClientUI().m_voaBillItem == null
				|| getClientUI().m_voaBillItem.length <= 0)
			return;
		...
		try {
			...
			nc.ui.ic.pub.pf.ICSourceRefBaseDlg.childButtonClicked(bo,
					getEnvironment().getCorpID(), getClientUI()
							.getFunctionNode(), getEnvironment().getUserID(),
			...
			if (!nc.ui.ic.pub.pf.ICSourceRefBaseDlg.makeFlag()) {
				...
				if (nc.ui.ic.pub.pf.ICSourceRefBaseDlg.isCloseOK()) {
					...
					// modified by liuzy 2009-8-21 上午10:33:20
					// 修改连接数，处理订单信息改到GeneralBillClientUI.transBillDataDeal
					...
					if (vos != null && vos.length > 0)
						...
						if (!ICConst.IsFromNewRef.equals(vos[0].getParentVO()
							...
							GenMethod.convertICAssistNumAtUI(
									(GeneralBillVO[]) vos,
									nc.ui.ic.pub.tools.GenMethod.getIntance());
						}
						// added by lirr 2009-02-21
						// 修改原因：非IsFromNewRef的转单需要设置nshouldoutassistnum精度
						else {
							for (GeneralBillVO vo : (GeneralBillVO[]) vos) {
								...
								sk
										.setAssistNumKeys(new String[] { "nshouldoutassistnum" });

								GenMethod.setScale(item, sk,
										getClientUI().m_ScaleValue);
							}
					...
					if (vos != null && vos.length == 1) {
						...
						getClientUI().setBillRefMultiVOs(sBusiTypeID,
								(GeneralBillVO[]) vos);
					}
					// end v5
	...
	protected void onAddLine() {
		...
		getClientUI().voBillAddLine(
				getBillCardPanel().getBillTable().getSelectedRow());
		nc.ui.ic.pub.tools.GenMethod.reSetRowColorWhenNOException(
				getBillCardPanel(), getBillCardPanel().getBillTable()
						.getSelectedRow());
		// 二次开发扩展
		try {
		...
		if (getM_voBill() != null && selrow != null) {
			for (int i = 0; i < selrow.length; i++) {
				if (getM_voBill().getItemBarcodeVO(selrow[i]) != null) {
					getClientUI().getM_utfBarCode().setRemoveBarcode(
							getM_voBill().getItemBarcodeVO(selrow[i]));
				}
			}
		}
	...
	protected void onSwitch() {
		...
		if (BillMode.List == getM_iCurPanel()) {
			...
			getClientUI().updateButton(
					getButtonTree().getButton(ICButtonConst.BTN_SWITCH));
			...
			if (getClientUI().m_sLastKey != null
				...
				if (getClientUI().m_bRefBills == true) {
					...
					try {
						// 修改人：刘家清 修改时间：2008-12-16 下午02:01:36
						// 修改原因：调拨出入库等单据，可以一次会生成多业务类型的单据，所以处理时要考虑把VO上业务类型设置给缓存。
						if (null != getM_voBill()
								&& null != getM_voBill().getBizTypeid())
							getClientUI().setBizTypeRef(
									getM_voBill().getBizTypeid());

						getClientUI().setBillRefResultVO(
								getClientUI().getBizTypeRef(),
	...
	protected void onDelete() {
		...
		try {
			...
			// 二次开发扩展
			getClientUI().getPluginProxy().beforeAction(
					nc.vo.scm.plugin.Action.DELETE, voaDeleteBill);
			...
			// 二次开发扩展
			getClientUI().getPluginProxy().afterAction(
					nc.vo.scm.plugin.Action.DELETE, voaDeleteBill);
			...
			while (true) {
				try {
					...
					nc.ui.pub.pf.PfUtilClient.processBatch("DELETE",
							getBillType(), getClientUI().getEnvironment()
									.getLogDate(), voaDeleteBill);
					break;
					...
					if (realbe != null
						...
						if (getClientUI().getAccreditLoginDialog().showModal() == nc.ui.pub.beans.UIDialog.ID_OK) {
							String sUserID = getClientUI()
									.getAccreditLoginDialog().getUserID();
							if (sUserID == null) {
	...
	protected void onQuery() {
		...
		try {
			...
			//
			// if (getClientUI().getConditionDlg().getResult() !=
			...
			// //nc.vo.pub.query.ConditionVO[] voaCond =
			// getClientUI().getConditionDlg()
			// // .getConditionVO();
			//			
			...
			// && (voaCond[i].getFieldCode().equals("head.dbilldate.from") ||
			// voaCond[i]
			...
			//			
			// /* yb query
			// //voCond.setParam(QryConditionVO.QRY_CONDITIONVO,
			// getClientUI().getAndConditionVO());
			// */
			//
			...
			if (!getClientUI().isBQuery() && getM_iCurPanel() != cardOrList) {
				onSwitch();
			}
		} catch (Exception e) {
			handleException(e);
	...
	protected void onJointCheck() {
		...
		if (getM_iLastSelListHeadRow() >= 0
			...
			nc.ui.scm.sourcebill.SourceBillFlowDlg soureDlg = new nc.ui.scm.sourcebill.SourceBillFlowDlg(
					getClientUI(), sBillTypeCode, sBillPK, null,
					getEnvironment().getUserID(),
	...
	protected void onUpdate() {
		...
		getClientUI().m_alLocatorDataBackup = getClientUI()
				.getM_alLocatorData();
		...
		if (!getClientUI().m_bIsEditBillCode
	...
	protected void onCancel() {
		...
		showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("common",
				"UCH045")/* @res "正在取消" */);
		if (-1 < getM_iLastSelListHeadRow()
				&& null != getM_alListData().get(getM_iLastSelListHeadRow()))
		...
		if (getM_iCurPanel() == BillMode.List) {
			if (getClientUI().m_bRefBills == true) {
				...
				getClientUI().setDataOnList(getClientUI().getM_alRefBillsBak(),
						false);
		...
		if (getM_iMode() != BillMode.Browse) {
			...
			// 恢复货位数据
			getClientUI().setM_alLocatorData(
					getClientUI().m_alLocatorDataBackup);
			// 清备份数据
			getClientUI().m_alLocatorDataBackup = null;
			...
			if (getM_voBill() != null)
				getClientUI().getM_utfBarCode().setAddBarcodes(
						getM_voBill().getItemVOs());
			setButtonStatus(false);
			// 根据是否为来源单据控制单据界面
			getClientUI().ctrlSourceBillUI(true);
	...
	protected void onCancelAudit() {
		try {
			...
			switch (nc.ui.pub.beans.MessageDialog.showYesNoDlg(getClientUI(),
					null, ResBase.getIsCancleSign()/* @res "是否确定要取消签字？" */
					, MessageDialog.ID_NO)) {
			...
			if (getM_voBill() != null) {
				...
				String message = getClientUI().checkBillForCancelAudit(
						getM_voBill());
				...
				// 二次开发扩展
				getClientUI().getPluginProxy().beforeAction(
						nc.vo.scm.plugin.Action.UNAUDIT,
						new GeneralBillVO[] { voAudit });
				while (true) {
					try {
			...
			e = nc.vo.ic.pub.GenMethod.getRealBusiException(e);
			handleException(e);
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	...
	protected void onAudit() {
		...
		try {
			...
			if (getM_voBill() != null) {
				...
				// 支持平台，clone一个，以便于以后的处理，同时防止修改了m_voBill
				GeneralBillVO voAudit = (GeneralBillVO) getM_voBill().clone();
				// modify by zhw 校验实际出库数量不能为零
				GeneralBillItemVO[] items = (GeneralBillItemVO[]) voAudit
						.getChildrenVO();
				if (getInOutFlag() < 0) {
					for (GeneralBillItemVO item : items) {
						if (UFDouble.ZERO_DBL.compareTo(PuPubVO
								.getUFDouble_NullAsZero(item.getNoutnum())) == 0) {
							showErrorMessage("第" + item.getCrowno()
									+ "行实际出库数量为零,不能审批");
							return;
						}
					}
				...
				// ---- old ------只要能保存，说明数量已经正确录入了。
				// -- new ---因为走平台，保存完整动作执行完后，还要重读是否签字了。
				String sBillStatus = getClientUI().afterAuditFrushData(voAudit,
						false);
				// 用新的方法替代,一次后台交互
				...
				// modify by zhw 2011-01-21
				// 调拨出库审核时，调拨订单实现行关闭，还原可用量和计划的累计领用数量，调拨出库不能弃审
				if (voAudit.getHeaderVO().getCbilltypecode().equalsIgnoreCase(
						ScmConst.m_allocationOut)) {
					Class[] ParameterTypes = new Class[] { GeneralBillVO.class };
					Object[] ParameterValues = new Object[] { voAudit };
					Object o = LongTimeTask.callRemoteService("scmpub",
							"nc.bs.hg.scm.pub.HgScmPubBO",
							"reWtriteToPlanFor4YClose", ParameterTypes,
							ParameterValues, 2);
				}
			...
			nc.ui.ic.pub.tools.GenMethod
					.reSetRowColorWhenNOException(getClientUI()
							.getBillCardPanel());
			...
			be = nc.vo.ic.pub.GenMethod.getRealBusiException(be);
			handleException(be);
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
			...
			if (be instanceof ICBusinessException) {
				...
				nc.ui.ic.pub.tools.GenMethod.setRowColorWhenException(
						getClientUI().getBillCardPanel(), ee);
			}
		}
	...
	protected void onCopy() {
		try {
			...
			getClientUI().qryLocSN(getM_iLastSelListHeadRow(),
					QryInfoConst.LOC_SN);
			// 显示复制的单据内容
			setM_voBill((GeneralBillVO) ((GeneralBillVO) getM_alListData().get(
					getM_iLastSelListHeadRow())).clone());
	...
	protected void onSpaceAssign() {
		...
		if (BillMode.Browse != getM_iMode())
			...
			showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
					"4008bill", "UPP4008bill-000022")/*
														 ...
														 * "出库的单据行时请先执行“序列号”，在序列号界面上汇总货位。"
														 */);
		// added by lirr 2009-11-19下午04:23:34
		// 增加参数getClientUI().isCheckAssetInv()
		getClientUI().getSpaceAllocationDlg().setData(getInOutFlag(), voWh,
				voInv, getM_iMode(), getClientUI().getM_alLocatorData(),
				getClientUI().getM_alSerialData(),
				getClientUI().isCheckAssetInv());
		...
		if (nc.ui.pub.beans.UIDialog.ID_OK == result
			...
			ArrayList alRes = getClientUI().getSpaceAllocationDlg()
					.getDataSpaceVO();
			// 保存
			...
			for (int row = 0; row < getM_voBill().getItemCount(); row++) {
				if (getM_voBill().getItemVOs()[row].getInOutFlag() == InOutFlag.IN) {
					alSnRes = getClientUI().getSpaceAllocationDlg()
							.getDataSNVO(row);
	...
	protected void onSNAssign() {
		...
		if ("40080822".equals(getClientUI().getFunctionNode())) {
			if (null != voBill.getItemValue(iCurSelBodyLine, "cwarehouseid")
					&& !voBill.getItemValue(iCurSelBodyLine, "cwarehouseid")
							.toString().equals(voWh.getCwarehouseid())) {
				int iQryMode = QryInfoConst.WH;
				// 参数
				Object oParam = voBill.getItemValue(iCurSelBodyLine,
						"cwarehouseid").toString();
				Object oRet;
				try {
					oRet = GeneralBillHelper.queryInfo(new Integer(iQryMode),
							oParam);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					nc.ui.ic.pub.tools.GenMethod.handleException(getClientUI(),
							null, e);
					return;
				}
				if (null != oRet) {
		...
		if (voInv != null && voBill != null) {
			...
			if (getClientUI().getM_alLocatorData() != null
					&& getClientUI().getM_alLocatorData().size() > 0)
				voLocs = (LocatorVO[]) getClientUI().getM_alLocatorData().get(
						iCurSelBodyLine);
			...
			if (getClientUI().getM_alSerialData() != null
					&& getClientUI().getM_alSerialData().size() > 0)
				serials = (SerialVO[]) getClientUI().getM_alSerialData().get(
						iCurSelBodyLine);
			else {
				serials = new SerialVO[] {};
				getClientUI().getM_alSerialData().add(iCurSelBodyLine, serials);
			}
			getClientUI().getSerialAllocationDlg().setDataVO(getInOutFlag(),
					voWh, aLoc, voInv, getM_iMode(), serials, voLocs);
			...
			if (nc.ui.pub.beans.UIDialog.ID_OK == result
					&& (BillMode.New == getM_iMode() || BillMode.Update == getM_iMode())) {
				SerialVO voaSN[] = getClientUI().getSerialAllocationDlg()
						.getDataSNVO();
				// 保存分配结果
				...
				// 如果是出库的话，还可能有货位分配数据
				ArrayList alRes = getClientUI().getSerialAllocationDlg()
						.getDataSpaceVO();
				if (alRes != null && alRes.size() > 0) {
					...
					// 保存到对应的行
					getClientUI().getM_alLocatorData().set(iCurSelBodyLine,
							voaLoc);
				}
	...
	protected void onSetpart() {
		...
		getClientUI().getSetpartDlg().setParam(getEnvironment().getCorpID(),
				sInvID);
	...
	protected void onOnHandShowHidden() {
		...
		getClientUI().setM_bOnhandShowHidden(
				!getClientUI().isM_bOnhandShowHidden());
	...
	protected void onPreview() {
		...
		try {
			...
			if (getM_iMode() == BillMode.Browse
				...
				nc.ui.pub.print.PrintEntry pe = getClientUI()
						.getPrintEntryNew();
				...
				GeneralBillVO[] destvoBills = (GeneralBillVO[]) getClientUI()
						.getPluginProxy().beforePrint(
								new GeneralBillVO[] { (GeneralBillVO) voBill
										.clone() });
				if (null != destvoBills && 0 < destvoBills.length
						&& destvoBills[0] instanceof GeneralBillVO)
	...
	private void printOnList(ArrayList alBill) throws InterruptedException {
		...
		// MessageDialog.showHintDlg(getClientUI(), null,
		// plc.getPrintResultMsg(false));
	}
	...
	protected void printOnCard(GeneralBillVO voBill, boolean isPreview) {
		...
		GeneralBillVO[] destvoBills = (GeneralBillVO[]) getClientUI()
				.getPluginProxy().beforePrint(
						new GeneralBillVO[] { (GeneralBillVO) voBill.clone() });
		if (null != destvoBills && 0 < destvoBills.length
				&& destvoBills[0] instanceof GeneralBillVO)
			getClientUI().getDataSource().setVO(destvoBills[0]);
		else
			// 向打印置入数据源，进行打印
			getClientUI().getDataSource().setVO(voBill);
		getClientUI().getPrintEntry().setDataSource(
				getClientUI().getDataSource());
	...
	protected void onPrintLocSN(GeneralBillVO voBill) {
		...
		for (int i = 0; i < voItems.length; i++) {
			...
			if (voItems[i].getLocator() != null) {
				...
				for (int j = 0; j < locs.length; j++) {
					...
					// 如果序列号不为空
					data[10] = getClientUI().getSNString(
							voItems[i].getSerial(), locs[j].getCspaceid());
					// modified by liuzy 2007-12-27 增加毛重主数量
			...
			// 如果货位为空，
				...
				data[10] = getClientUI().getSNString(voItems[i].getSerial(),
						null);
				// modified by liuzy 2007-12-27 增加毛重主数量
				data[11] = voItems[i].getAttributeValue(getEnvironment()
						.getGrossNumItemKey());
		...
		if (v.size() > 0) {
			...
			//
			// nc.ui.pub.print.PrintDirectEntry print = new
			// nc.ui.pub.print.PrintDirectEntry();
	...
	protected void onCheckData() {
		try {
			...
			java.util.ArrayList alResult = getClientUI().getDevInputCtrl()
					.onOpenFile(getClientUI().getDevInputCtrl().ACT_CHECK_ITEM);
	...
	protected void onImportData() {
		try {
			...
			nc.ui.ic.pub.device.DevInputCtrl devInputCtrl = getClientUI()
					.getDevInputCtrl();
			...
			if (iAppendType == DevInputCtrl.ACT_ADD_ITEM) {
				if (voaDi != null && voaDi.length > 0) {
					...
					// 同步vo.
					getClientUI().synVO(voaDi, sWarehouseid,
							getClientUI().getM_alLocatorData());
	...
	protected void onImptBCExcel(int iMenu) {
		...
		try {
			...
			if (m_eDlg.isExportOK()) {
				...
				if (voaImport == null || voaImport.length == 0
						|| voaImport[0] == null) {
					MessageDialog.showErrorDlg(getClientUI(),
							nc.ui.ml.NCLangRes.getInstance().getStrByID(
									"SCMCOMMON", "UPPSCMCommon-000059")/*
																		 * @res
																		 * "错误"
																		 */, nc.ui.ml.NCLangRes.getInstance().getStrByID(
									"4008bill", "UPP4008bill-000088")/*
																		 * @res
																		 * "Excel文件条码为空！"
																		 */);
					return;
		...
		if ((getM_voBill().getHeaderVO().getPrimaryKey() != null)
			...
			if ((null != voaImport[0].getBasstaddflag() && voaImport[0]
					.getBasstaddflag().booleanValue())
					|| (null != voaImport[0].getBasstaddflagsub() && voaImport[0]
							.getBasstaddflagsub().booleanValue())) {
				num = (UFDouble) (billItemvo.getAttributeValue(getEnvironment()
						.getAssistNumItemKey()));
			...
			if ((null != voaImport[0].getBasstaddflag() && voaImport[0]
					...
					|| (null != voaImport[0].getBasstaddflagsub() && voaImport[0]
							.getBasstaddflagsub().booleanValue())) {
				// if (null ==
				// getM_voBill().getItemVOs()[rownow].getAttributeValue(getEnvironment().getAssistNumItemKey()))
				getM_voBill().getItemVOs()[rownow].setAttributeValue(
						getEnvironment().getAssistNumItemKey(), new UFDouble(
								allNum));
				/*
				 * else
				 * getM_voBill().getItemVOs()[rownow].setAttributeValue(getEnvironment().getAssistNumItemKey(),(((UFDouble)
				 * getM_voBill().getItemVOs()[rownow].getAttributeValue(getEnvironment().getAssistNumItemKey())).add(new
				 * UFDouble(allNum))));
				 */getBillCardPanel().setBodyValueAt(new UFDouble(allNum),
						rownow, getEnvironment().getAssistNumItemKey());
				BillEditEvent ev = new BillEditEvent(getBillCardPanel()
						.getBodyItem(getEnvironment().getAssistNumItemKey()),
						null, new UFDouble(allNum), getEnvironment()
								.getAssistNumItemKey(), rownow, BillItem.BODY);
				getClientUI().afterEdit(ev);
			} else {
				// if (null ==
				// getM_voBill().getItemVOs()[rownow].getAttributeValue(getEnvironment().getNumItemKey()))
				getM_voBill().getItemVOs()[rownow].setAttributeValue(
						getEnvironment().getNumItemKey(), new UFDouble(allNum));
				/*
				 * else
				 * getM_voBill().getItemVOs()[rownow].setAttributeValue(getEnvironment().getNumItemKey(),(((UFDouble)
				 * getM_voBill().getItemVOs()[rownow].getAttributeValue(getEnvironment().getNumItemKey())).add(new
				 * UFDouble(allNum))));
				 */getBillCardPanel().setBodyValueAt(new UFDouble(allNum),
						rownow, getEnvironment().getNumItemKey());
				BillEditEvent ev = new BillEditEvent(getBillCardPanel()
						.getBodyItem(getEnvironment().getNumItemKey()), null,
		...
		try {
			if ((getM_voBill().getHeaderVO().getPrimaryKey() != null)
					&& (getM_iMode() == BillMode.Browse)) {
				if (getClientUI()
						.onImportSignedBillBarcode(getM_voBill(), true) == 1)
					showHintMessage(nc.ui.ml.NCLangRes.getInstance()
				...
				for (int n = 0; n < alBill.size(); n++) {
					...
					if (sBillTypecode != null
							&& (sBillTypecode
									...
									|| sBillTypecode
											.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_allocationIn) || sBillTypecode
					...
					try {
						...
						// false:前台不作校验，后台校验
						if (getM_iMode() == BillMode.Browse) {
							getClientUI().onImportSignedBillBarcode(billVO,
									false);
							showHintMessage(nc.ui.ml.NCLangRes.getInstance()
									.getStrByID("4008bill",
											"UPP4008bill-000083")/*
	...
	protected void onBarcodeOpenClose(int iChoose) {
		...
		getClientUI().updateBillToListByItemKeys(
				getM_voBill(),
				null,
				new String[] { nc.ui.ic.pub.bill.ItemKeyS.bbarcodeclose,
	...
	protected void onBarCodeEdit() {
		...
		if (getM_iMode() == BillMode.Browse
				|| !getClientUI().m_bAddBarCodeField) {
			bDirectSave = true;
		} else {
			bDirectSave = false;
		}
		...
		getClientUI().m_dlgBarCodeEdit = getClientUI().getBarCodeDlg(bEditable,
				bDirectSave);
		...
		if (voBill != null && iCurFixLine >= 0
			...
			if (itemvo.getBarcodeManagerflag().booleanValue()) {
				...
				getClientUI().getGenBillUICtl().setBillCrowNo(voBill,
						getBillCardPanel());

				// 获得条码管理的Items

				ArrayList alReuslt = getClientUI().getBarcodeCtrl()
						.getCurBarcodeItems(voBill, iCurFixLine);
				...
				getClientUI().m_dlgBarCodeEdit
						.setColor(getClientUI().m_sColorRow);
				getClientUI().m_dlgBarCodeEdit
						.setScale(getClientUI().m_ScaleValue
								.getScaleValueArray());

				// 将条码是否保存参数设置到条码编辑界面，便于在编辑界面保存条码前控制
				// m_dlgBarCodeEdit.setSaveBarCode(m_bBarcodeSave);
				getClientUI().m_dlgBarCodeEdit
						.setSaveBadBarCode(getClientUI().m_bBadBarcodeSave);
				...
				getClientUI().m_dlgBarCodeEdit
						.setUIEditableBarcodeClose(!bbarcodeclose);
				getClientUI().m_dlgBarCodeEdit.setUIEditable(getM_iMode());
				// 设置到Items中
				getClientUI().m_dlgBarCodeEdit.setCurBarcodeItems(
						itemBarcodeVos, iFixLine);
				...
				if (getClientUI().m_dlgBarCodeEdit.showModal() == nc.ui.pub.beans.UIDialog.ID_OK) {
					...
					// 设置条码框数据
					getClientUI().getM_utfBarCode().setCurBillItem(
							itemBarcodeVos);
					...
					Hashtable htbItemBarcodeVos = getClientUI()
							.getBarcodeCtrl().getHtbItemBarcodeVos(
									itemBarcodeVos);
					...
					if (htbItemBarcodeVos != null
						...
						if (htbItemBarcodeVos != null) {
							...
							if (getClientUI().m_dlgBarCodeEdit.m_bModifyBillUINum
									&& getClientUI().getBarcodeCtrl()
											.isModifyBillUINum()) { // 修改卡片界面状态
								showHintMessage(nc.ui.ml.NCLangRes
										.getInstance().getStrByID("4008bill",
												"UPP4008bill-000075")/*
																		 * @res
				...
				nc.ui.ic.pub.tools.GenMethod.handleException(getClientUI(),
						null, new BusinessException(nc.ui.ml.NCLangRes
								.getInstance().getStrByID("4008bill",
										"UPP4008bill-000002")/*
																 * @res
	...
	private void onBarCodeEditUpdateBill(int iCurFixLine,
		...
		if (getClientUI().m_dlgBarCodeEdit.m_bModifyBillUINum
			...
			// 执行模版公式
			getClientUI().getGenBillUICtl().execEditFormula(getBillCardPanel(),
	...
	protected void onDocument() {
		...
		DocumentManager
				.showDM(getClientUI(), vo.getCbilltypecode(), spk/* , scode */);
	}
	...
	protected void onPickAuto() {
		...
		try {
			...
			voOutBill.getHeaderVO().setCoperatoridnow(
					getEnvironment().getUserID());
			voOutBill.getHeaderVO().setClogdatenow(
					getEnvironment().getLogDate());
			boolean isSN = false;
			boolean isLocator = false;
			try {
				voNew = nc.ui.ic.ic2a1.PickBillHelper
						.pickAuto(voOutBill, new nc.vo.pub.lang.UFDate(
								getEnvironment().getLogDate()));
				if (voNew != null && voNew.getItemVOs() != null) {
					...
					// 执行公式
					getBillCardPanel().getBillModel().execFormulasWithVO(
							voItems, getClientUI().getBodyFormula());
				...
				nc.ui.pub.beans.MessageDialog
						.showErrorDlg(getClientUI(), nc.ui.ml.NCLangRes
								.getInstance().getStrByID("SCMCOMMON",
										"UPPSCMCommon-000059")/* @res "错误" */,
								nc.ui.ml.NCLangRes.getInstance().getStrByID(
										"4008bill", "UPP4008bill-000118")/*
																			 * @res
																			 * "自动拣货失败："
																			 */
										+ e.getMessage());
			...
			if (voItems != null && voItems.length > 0) {
				...
				for (int i = 0; i < voItems.length; i++) {
					...
					voItems[i].procLocNumDigit(getClientUI().m_ScaleValue
							.getNumScale(), getClientUI().m_ScaleValue
							.getAssistNumScale());
					if (!ht.containsKey(key)) {// 如果是原行
						...
						// deleted by lirr 2009-06-02 修改原因：NCdp200867926
						// 存货多行时部分序列号没有选中
						// voRow.setSerial(null);
						voRow.setAttributeValue("cparentid", null);
						voRow.setBarCodeVOs(null);
						voRow.setAttributeValue(IItemKey.NBARCODENUM,
								new UFDouble(0.0));
						voRow
								.setBarcodeClose(new nc.vo.pub.lang.UFBoolean(
										'N'));
						...
						if (dkey.compareTo(dLastRowNo) < 0) {
							...
							getBillCardPanel().getBillModel().setBodyRowVO(
									voRow, iCurRow);
							getClientUI().execEditFomulas(iCurRow,
									getEnvironment().getNumItemKey());
						} else {
							// 增行
							getBillCardPanel().addLine();
			 ...
			 * if (selrow >= 0) { nc.ui.pub.bill.BillEditEvent event = new
			 * nc.ui.pub.bill.BillEditEvent( getBillCardPanel().getBillTable(),
			 * selrow, selrow); getClientUI().bodyRowChange(event); }
			 */
			//
			return;
		} finally {
	...
	protected void onRefInICBill() {
		try {
			...
			dlgQry.initData(getEnvironment().getCorpID(), getEnvironment()
					.getUserID(), "40089908", null, "4I",
					nc.vo.ic.pub.BillTypeConst.m_otherIn, null);
			...
			if (dlgQry.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
				...
				nc.ui.ic.pub.pf.ICSourceRefBaseDlg dlgBill = new ICSourceRefBaseDlg(
						...
						getEnvironment().getUserID(), "40080608", sWhere
								.toString(),
						nc.vo.ic.pub.BillTypeConst.m_otherIn, null, null, "4I",
						getClientUI());
				class IBillReferQueryProxyExt extends IBillReferQueryProxy {

					public IBillReferQueryProxyExt(
							nc.ui.ic.pub.pf.ICBillQuery adlg) {
						this.dlg = adlg;
					}
				...
				if (dlgBill.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
					...
					/*
					 * nc.vo.pub.AggregatedValueObject[] voRetvos =
					 * (nc.vo.pub.AggregatedValueObject[])
					 * nc.ui.pub.change.PfChangeBO_Client
					 * .pfChangeBillToBillArray(vos,
					 * nc.vo.ic.pub.BillTypeConst.m_otherIn, "4I");
					 */
	...
	protected void onBillExcel(int iFlag) {
		...
		if (iFlag == 1 || iFlag == 3/* 导出为xml */) {
			...
			if (getClientUI().getChooser().showSaveDialog(getClientUI()) == javax.swing.JFileChooser.CANCEL_OPTION)
				return;
			sFilePathDir = getClientUI().getChooser().getSelectedFile()
					.toString();
		}
	...
	private void onExportXML(GeneralBillVO[] billvos, String filename) {
		if (billvos == null || billvos.length <= 0 || filename == null)
			return;
		try {
			MessageDialog.showInputDlg(getClientUI(), "请输入外部系统编码",
					"请输入外部系统编码:", "20", 5);
			...
			Document outdocs = export.exportBills(billvos,
					nc.ui.pub.ClientEnvironment.getInstance().getAccount()
							.getAccountCode(), nc.ui.pub.ClientEnvironment
	...
	protected void onMergeShow() {
		...
		CollectSettingDlg dlg = new CollectSettingDlg(getClientUI(),
				getBillType(), getClientUI().getFunctionNode(), getClientUI()
						.getEnvironment().getCorpID(), getClientUI()
						.getEnvironment().getUserID(), GeneralBillVO.class
						.getName(), GeneralBillHeaderVO.class.getName(),
				GeneralBillItemVO.class.getName());
	...
	protected void onAddNewRowNo() {
		...
		for (int i = 0; i < getBillCardPanel().getRowCount(); i++) {
			if (getBillCardPanel().getBillModel().getRowState(i) == BillModel.NORMAL) {
				...
				getM_voBill().setItemValue(i, "crowno",
						getBillCardPanel().getBodyValueAt(i, "crowno"));
			}
		}
	}
	...
	protected void onLineCardEdit() {
		...
		try {
			...
			if (getButton(ICButtonConst.BTN_LINE_ADD) != null
					&& getButton(ICButtonConst.BTN_LINE_ADD).isEnabled())
				isenableaddrow = true;
	...
	protected void onDMStateQuery() {
		...
		try {
			dlgclass = Class
					.forName("nc.ui.dm.service.delivery.SourceBillDeliveryStatusUI");
			dlg = dlgclass.getConstructor(new Class[] { ToftPanel.class })
					.newInstance(new Object[] { getClientUI() });
		} catch (Throwable e) {
			showErrorMessage("运输没有安装");
			return;
		}

		HashMap hsdata = nc.ui.ic.pub.tools.GenMethod.getValues(
				"ic_general_bb3", new String[] { IItemKey.ntotaltrannum,
						IItemKey.btranendflag }, new int[] {
						SmartFieldMeta.JAVATYPE_UFDOUBLE,
						SmartFieldMeta.JAVATYPE_UFBOOLEAN },
				IItemKey.CGENERALBID, SmartVOUtilExt.getVOsValues(vo
						.getChildrenVO(), IItemKey.CGENERALBID), null);
		...
		SourceBillDeliveryStatus[] qrydatas = new SourceBillDeliveryStatus[vo
				.getItemCount()];
		for (int i = 0; i < qrydatas.length; i++) {
			...
			qrydatas[i].setCbill_bid((String) vo.getItemValue(i,
					IItemKey.CGENERALBID));
			qrydatas[i].setCinvbasid((String) vo.getItemValue(i,
					IItemKey.INVBASID));
			qrydatas[i].setCrowno((String) vo.getItemValue(i, IItemKey.CROWNO));
			qrydatas[i].setCunitid((String) vo.getItemValue(i, "pk_measdoc0"));
			qrydatas[i]
					.setCunitname((String) vo.getItemValue(i, "measdocname"));
			...
			if (hsdata != null && hsdata.size() > 0) {
				Object[] row = (Object[]) hsdata
						.get(qrydatas[i].getCbill_bid());
				if (row != null && row.length > 0) {
					qrydatas[i].setNsendnum((UFDouble) row[0]);
					qrydatas[i].setBsendendflag((UFBoolean) row[1]);
				}
			}
			qrydatas[i].setNcansendnum(SmartVOUtilExt.sub(qrydatas[i]
					.getNnumber(), qrydatas[i].getNsendnum() == null ? zero
					: qrydatas[i].getNsendnum()));
		}

		try {

			nc.ui.ic.pub.tools.GenMethod.callMethod(dlg, "show",
					new Class[] { qrydatas.getClass() },
					new Object[] { qrydatas });
	...
	public boolean onEditAction(int action) {
		...
		boolean isSort = getBillCardPanel().getBillTable().isSortEnabled();
		BillActionListener bal = getBillCardPanel().getBodyPanel()
				.getBillActionListener();
		try {
			if (bal != null)
	 ...
	 * @author zhw
	 * @throws Exception
	 * @说明：（鹤岗矿业） 2011-3-7上午09:37:03
	 */
	public void getInfoByBatchCode() throws Exception {
		
		if (getM_voBill() == null)
			throw new BusinessException("获取缓存数据出错");
		
		String loginCorp = ClientEnvironment.getInstance().getCorporation().getPk_corp();
		GeneralBillHeaderVO head = getM_voBill().getHeaderVO();
		
		GeneralBillItemVO[] items = (GeneralBillItemVO[]) getM_voBill().getChildrenVO();
		String billtype = getM_voBill().getCbilltypecode();
		...
		for (int i = 0; i < len; i++) {
			...
			GeneralBillItemVO item = items[i];
			String vbatchcode = item.getVbatchcode();
			
			String cinventoryid = item.getCinventoryid();
			String invbasid = item.getCinvbasid();
			
			if (vbatchcode == null || cinventoryid == null)
				throw new BusinessException("批次,存货出错");

			UFDouble price = UFDouble.ZERO_DBL;
			infor = vbatchcode + "&" + cinventoryid + "&" + invbasid + "&"+ loginCorp;
			
			al.add(infor);
		}
		
		o = HgICPubHealper.getInfoByBatchCode(al);
		
		if (o != null) {
			
			ArrayList alc = (ArrayList) o;
			int size = alc.size();
			
			for (int i = 0; i < size; i++) {
				
				if (HgPubConst.GYC_CORPID.equalsIgnoreCase(loginCorp)) {
					
					getBillCardPanel().getBillModel().setValueAt(PuPubVO.getString_TrimZeroLenAsNull(alc.get(i)), i,"cvendorid");
					getBillCardPanel().execBodyFormula(i, "vvendorname");
				
				} else {
					
					if (ScmConst.m_materialOut.equalsIgnoreCase(billtype)) {
						
						UFDouble price = PuPubVO.getUFDouble_NullAsZero(alc.get(i));
						UFDouble num =PuPubVO.getUFDouble_NullAsZero(getBillCardPanel().getBillModel().getValueAt(i,"noutnum"));
						
						getBillCardPanel().getBillModel().setValueAt(price, i,"nprice");
						getBillCardPanel().getBillModel().setValueAt(price.multiply(num),i, "nmny");
					}
				}
			}
		}
