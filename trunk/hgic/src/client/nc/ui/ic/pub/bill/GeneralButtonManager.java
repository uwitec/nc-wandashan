...
//		      二次开发扩展
	...
	protected void onAudit() {
        ...
        GeneralBillVO.setBillBCVOStatus(getM_voBill(), nc.vo.pub.VOStatus.UNCHANGED);
				...
				GeneralBillItemVO[] items = (GeneralBillItemVO[])voAudit.getChildrenVO();
				if(getInOutFlag()<0){
					for(GeneralBillItemVO item:items){
						if(UFDouble.ZERO_DBL.compareTo(PuPubVO.getUFDouble_NullAsZero(item.getNoutnum()))==0){
							showErrorMessage("第"+item.getCrowno()+"行实际出库数量为零,不能审批");
							return;
						}
					}
				}
...
//      二次开发扩展
        ...
        if(voAudit.getHeaderVO().getCbilltypecode().equalsIgnoreCase(ScmConst.m_allocationOut)){
        	Class[] ParameterTypes = new Class[]{GeneralBillVO.class};
    		Object[] ParameterValues = new Object[]{voAudit};
    		Object o=LongTimeTask.callRemoteService("scmpub","nc.bs.hg.scm.pub.HgScmPubBO", "reWtriteToPlanFor4YClose", ParameterTypes, ParameterValues, 2);
        }
	   ...
	   for(int i=0;i<len;i++){
		   ...
		   try { 
			   if(ScmConst.m_materialOut.equalsIgnoreCase(billtype)){
				   Object o=HgICPubHealper.getInfoByBatchCode(cinventoryid, vcode, corp, vbatchcode);
				   if(o !=null){
					   Object[] os = (Object[])o;
					   cvendor =PuPubVO.getString_TrimZeroLenAsNull(os[0]);
					   price =PuPubVO.getUFDouble_NullAsZero(os[1]);
				   }
			   }else{
				   cvendor = HgICPubHealper.getcvendor(cinventoryid,vbatchcode);
			   }
	    	...
	    	if(cvendor !=null){
	    	 String pk_cubasdoc = execFormular("getColValue(bd_cumandoc,pk_cubasdoc,pk_cumandoc,cvendorid)",cvendor);
	  	     String vvendorname =execFormular("getColValue(bd_cubasdoc,custname,pk_cubasdoc,pk_cubasdoc)",pk_cubasdoc);
	    	 getM_voBill().setItemValue(i, "cvendorid", cvendor);
	  	     getM_voBill().setItemValue(i, "pk_cubasdoc",pk_cubasdoc);
	  	     getBillCardPanel().setBodyValueAt(cvendor, i, "cvendorid");
	  	     getBillCardPanel().setBodyValueAt(pk_cubasdoc, i, "pk_cubasdoc");
	  	     getBillCardPanel().setBodyValueAt(vvendorname, i, "vvendorname");
	    	}
	    	
	    	if(!UFDouble.ZERO_DBL.equals(price)){
	    		getBillCardPanel().setBodyValueAt(price, i, "nprice");
		  	    getBillCardPanel().setBodyValueAt(price.multiply(num), i, "nmny");
	    	}
	   }
  }
