...
public class BiEvaluationBodyVO extends SuperVO {
	...
	public void validataOnSplitNum() throws ValidationException{
		...
		if(PuPubVO.getUFDouble_NullAsZero(getNzbnum()).equals(UFDouble.ZERO_DBL)){
			throw new ValidationException("招标数量为空，行号为"+getCrowno());
		}
		if(PuPubVO.getUFDouble_NullAsZero(getNzbprice()).equals(UFDouble.ZERO_DBL)){
			throw new ValidationException("中标价格为空，行号为"+getCrowno());
		}
		
		BidSlvendorVO[] vendors = getBidSlvendorVOs();
		if(vendors==null || vendors.length ==0)
			throw new ValidationException("入围供应商为空，行号为"+getCrowno());
		for(BidSlvendorVO vendor:vendors){
			vendor.validataOnSplitNum();
		}
	}
	
	public void validataOnSaveSplitNum() throws ValidationException{
		BidSlvendorVO[] vendors = getBidSlvendorVOs();
		if(vendors==null || vendors.length ==0)
			throw new ValidationException("入围供应商为空，行号为"+getCrowno());
		UFDouble nzbnum = UFDouble.ZERO_DBL;
		for(BidSlvendorVO vendor:vendors){
			vendor.validataOnSaveSplitNum(getNzbprice());
			nzbnum = nzbnum.add(PuPubVO.getUFDouble_NullAsZero(vendor.getNzbnum()));
		}
		if(!nzbnum.equals(getNzbnum()))
			throw new ValidationException("招标数量存在剩余数量,招标数量为："+getNzbnum().toString()+"供应商中标数量之和为："+nzbnum.toString());
    ...
    /**
	  ...
	  * 创建日期:2011-05-04 11:52:10
	  */
     public BiEvaluationBodyVO() {
		super();	
	}
	public BidSlvendorVO[] getBidSlvendorVOs() {
		return bidSlvendorVOs;
	}
	public void setBidSlvendorVOs(BidSlvendorVO[] bidSlvendorVOs) {
		this.bidSlvendorVOs = bidSlvendorVOs;
	}
	...
	public void validate() throws ValidationException {
		if(PuPubVO.getString_TrimZeroLenAsNull(getCrowno())==null){
			throw new ValidationException("存在行号为空的表体行");
		}
//		if(PuPubVO.getString_TrimZeroLenAsNull(getCinvclid())==null){
//			throw new ValidationException("存货分类为空，行号:"+getCrowno());
//		}
		if(PuPubVO.getString_TrimZeroLenAsNull(getCunitid())==null)
			throw new ValidationException("主计量单位为空，行号："+getCrowno());
		if(PuPubVO.getUFDouble_NullAsZero(getNzbnum()).equals(UFDouble.ZERO_DBL))
			throw new ValidationException("招标数量为空，行号："+getCrowno());
		if(PuPubVO.getUFDouble_NullAsZero(getNzbprice()).equals(UFDouble.ZERO_DBL))
			throw new ValidationException("中标价为空，行号："+getCrowno());
		
	}
	public void validate(boolean isinv) throws ValidationException{
		validate();
		if(isinv)
			if(PuPubVO.getString_TrimZeroLenAsNull(getCinvbasid())==null||PuPubVO.getString_TrimZeroLenAsNull(getCinvmanid())==null){
				throw new ValidationException("存货为空，行号："+getCrowno());
			}
	}
} 
