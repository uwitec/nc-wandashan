...
public class SendBillEventHandler extends WdsPubEnventHandler {
	...
	protected UIDialog createQueryUI() {
		return new HYQueryDLG(getBillUI(), null, getBillUI()._getCorp().getPrimaryKey(), WdsWlPubConst.TRANS_PRICE_NODECODE, getBillUI()._getOperator(), null);
	}
	...
	protected String getHeadCondition() {
		...
		if(strWhere == null || "".equals(strWhere)){
			return " and pk_billtype='"+WdsWlPubConst.WDSM+"'";
		}else {
			return strWhere+" and pk_billtype='"+WdsWlPubConst.WDSM+"'";
		}
	}
	@Override
	protected void onBoElse(int intBtn) throws Exception {
		...
		switch (intBtn){
			...
			((SendBillUI)getBillUI()).setRefBillType(WdsWlPubConst.BILLTYPE_SALE_OUT);
			onBillRef();
			break;
			case ButtonCommon.REFWDS10:
			((SendBillUI)getBillUI()).setRefBillType(WdsWlPubConst.WDS3);
			onBillRef();	
			break;
			case ButtonCommon.REFWDS11:
				((SendBillUI)getBillUI()).setRefBillType(WdsWlPubConst.WDS5);
				onBillRef();	
			break;	
