...
public class PlanPubBO {
	...
	public PlanApplyInforVO getPlanAppInfor(String sLogCorp, String sLogUser)
			throws BusinessException {
		PlanApplyInforVO infor = null;

		// 申请人 申请部门 申请组织
		String sql = " select psn.pk_psndoc capplypsnid,psn.pk_deptdoc capplydeptid,dep.pk_calbody creqcalbodyid,dep.pk_fathedept csupplydeptid"
		...
		Object o = getBaseDao().executeQuery(sql,
				new BeanProcessor(PlanApplyInforVO.class));
		if (o == null) {
		...
		if (!infor.getM_pocorp().equalsIgnoreCase(sLogCorp)) {// 当前公司不是集采公司
			if (PuPubVO.getString_TrimZeroLenAsNull(infor.getCsupplydeptid()) == null) {// 当前部门为采购部门
				...
				// 部门 和 组织 补丁 等待 实施方案
				// 供应处只有一个组织默认带出
				String calbody = PuPubVO
						.getString_TrimZeroLenAsNull(HgBsPubTool
								.execFomular(
										"pk_calbody->getColValue(bd_calbody,pk_calbody,pk_corp,corpid)",
										new String[] { "corpid" },
										new String[] { infor.getM_pocorp() }));
				if (calbody != null)
					infor.setCsupplycalbodyid(calbody);
			} else {
				sql = "select pk_corp,pk_calbody from bd_deptdoc where pk_deptdoc = '"
						+ infor.getCsupplydeptid() + "'";
				o = getBaseDao().executeQuery(sql, HgBsPubTool.ARRAYPROCESSOR);
				if (o == null)
					throw new BusinessException("后台数据查询异常，获取部门档案信息出错");
				Object[] os = (Object[]) o;
				infor.setCsupplycorpid(PuPubVO
						.getString_TrimZeroLenAsNull(os[0]));
				infor.setCsupplycalbodyid(PuPubVO
						.getString_TrimZeroLenAsNull(os[1]));
			}
		}
		return infor;
	}
	...
	private boolean isHead(HgYearExcelFileVO vo) {
		return vo.getDbilldate() == null && vo.getPk_corp() == null
				&& vo.getCapplydeptid() == null && vo.getCapplypsnid() == null
				&& vo.getHcreqcalbodyid() == null && vo.getFisself() == null
				&& vo.getCsupplycorpid() == null
				&& vo.getCsupplydeptid() == null && vo.getCyear() == null
				&& vo.getHvmemo() == null;

	}

	private PlanVO getHeadVoForImport(HgYearExcelFileVO vo, String userid,
			int index) throws BusinessException {
		...
		if (vo.getPk_corp() != null) {
			pk_corp = querypkall("pk_corp", "bd_corp", "unitcode", vo
					.getPk_corp(), "");
			...
			String sql = "select cuserid from sm_user where dr = 0 and cuserid='"
					+ userid + "'and pk_corp = '" + pk_corp + "'";
			Object cuserid = getBaseDao().executeQuery(sql,
		...
		if (vo.getCapplydeptid() != null) {
			Object pk_deptdoc = querypkall("pk_deptdoc", "bd_deptdoc",
					"deptcode", vo.getCapplydeptid(), pk_corp.toString());
			if (PuPubVO.getString_TrimZeroLenAsNull(pk_deptdoc) == null)
				errorlog += "第" + index + "行,部门" + vo.getCapplydeptid()
						+ "不存在\n";
			else
		...
		if (vo.getCapplypsnid() != null) {
			Object pk_psndoc = querypkall("pk_psndoc", "bd_psndoc", "psncode",
					vo.getCapplypsnid(), "");
			if (PuPubVO.getString_TrimZeroLenAsNull(pk_psndoc) == null)
				errorlog += "第" + index + "行,申请人" + vo.getCapplypsnid()
						+ "不存在\n";
			else
		...
		if (vo.getHcreqcalbodyid() != null) {
			Object pk_calbody = querypkall("pk_calbody", "bd_calbody",
					"bodycode", vo.getHcreqcalbodyid(), "");
			if (PuPubVO.getString_TrimZeroLenAsNull(pk_calbody) == null)
				errorlog += "第" + index + "行,需求组织" + vo.getHcreqcalbodyid()
						+ "不存在\n";
			else
		...
		if (vo.getCsupplycorpid() != null) {
			Object csupplycorpid = querypkall("pk_corp", "bd_corp", "unitcode",
					vo.getCsupplycorpid(), "Y");
			if (csupplycorpid != null)
				headvo.setCsupplycorpid(csupplycorpid.toString());
		}
		// 供货部门
		if (vo.getCsupplydeptid() != null) {
			Object csupplydeptid = querypkall("pk_deptdoc", "bd_deptdoc",
					"deptcode", vo.getCsupplydeptid(), "Y");
	...
	private PlanYearBVO getBodyVoForImport(HgYearExcelFileVO vo, int index)
			throws BusinessException {
		...
		if (vo.getInvcode() != null) {
			String sql = "select man.pk_invmandoc, man.pk_invbasdoc from bd_invmandoc man,bd_invbasdoc doc where man.pk_invbasdoc = doc.pk_invbasdoc and man.dr =0 and"
					+ " doc.dr=0 and doc.invcode = '"
					+ vo.getInvcode()
					+ "' and man.pk_corp = '" + SQLHelper.getCorpPk() + "'";
			Object list = getBaseDao().executeQuery(sql,
					HgBsPubTool.ARRAYLISTPROCESSOR);
			if (list != null) {
				ArrayList ar = (ArrayList) list;
				if (ar.size() == 0)
					errorlog += "第" + index + "行,编码:" + vo.getInvcode()
							+ ",未找到该存货\n";
				else {
		...
		if (vo.getBcreqcalbodyid() != null) {
			Object bpk_calbody = querypkall("pk_calbody", "bd_calbody",
					"bodycode", vo.getBcreqcalbodyid(), "");
			if (PuPubVO.getString_TrimZeroLenAsNull(bpk_calbody) == null)
				errorlog += "第" + index + "行,表体需求组织" + vo.getBcreqcalbodyid()
						+ "不存在\n";
			else
		...
		if (vo.getCreqwarehouseid() != null) {
			Object pk_stordoc = querypkall("pk_stordoc", "bd_stordoc",
					"storcode", vo.getCreqwarehouseid(), "");
			if (PuPubVO.getString_TrimZeroLenAsNull(pk_stordoc) == null)
				errorlog += "第" + index + "行,需求仓库" + vo.getCreqwarehouseid()
						+ "不存在\n";
			else
		...
		if (vo.getCsupplycalbodyid() != null) {
			Object csupplycalbodyid = querypkall("pk_calbody", "bd_calbody",
					"bodycode", vo.getCsupplycalbodyid(), "Y");
			if (PuPubVO.getString_TrimZeroLenAsNull(csupplycalbodyid) == null)
				errorlog += "第" + index + "行,供货组织" + vo.getCsupplycalbodyid()
						+ "不存在\n";
			else
		...
		if (vo.getCsupplywarehouseid() != null) {
			Object csupplywarehouseid = querypkall("pk_stordoc", "bd_stordoc",
					"storcode", vo.getCsupplywarehouseid(), "Y");
		...
		if (vo.getPk_measdoc() != null) {
			Object pk_measdoc = querypkall("pk_measdoc", "bd_measdoc",
					"shortname", vo.getPk_measdoc(), "Y");
			if (pk_measdoc != null)
	 ...
	 * @throws DAOException
	 */
	public void vHgChangeVo(nc.vo.hg.pu.pub.HgYearExcelFileVO[] vos,
		...
		HYBillVO[] newbills = (HYBillVO[]) pushSavePlans(lbillvo
				.toArray(new HYBillVO[0]));

		for (HYBillVO newbill : newbills) {
			getYearPlanBO().splitYearPlan2MonthPlan(newbill,
					SQLHelper.getCorpPk());
		}
	}
	...
	public Object querypkall(String selectname, String tablename,
			String codename, String passvalue, String bag) throws DAOException {
		String sqlpkcorp = "select " + selectname + " from " + tablename
				+ " where dr = 0 and " + codename + " = '" + passvalue;
		if (bag.equals("Y"))
			sqlpkcorp += "';";
		...
		Object obj = getBaseDao().executeQuery(sqlpkcorp,
				HgBsPubTool.COLUMNPROCESSOR);
		return obj;
	}
	...
	public Object pushSavePlans(HYBillVO[] billvos) throws BusinessException {
		...
		if (isimport) {
			...
			String[] nos = HgPubTool.getBatchBillNo(
					HgPubConst.PLAN_YEAR_BILLTYPE, pk_corp, null, intNum);
			int index = 0;
			PlanVO head = null;
			for (HYBillVO billvo : billvos) {
				...
				// 设置默认会计期间
				head.setCaccperiodschemeid(AccountCalendar.getInstance()
						.getYearVO().getPk_accperiodscheme());
				index++;
			}
		}
		...
		for (HYBillVO billvo : billvos) {
			...
			for (PlanBVO body : tmpbodys) {
				...
				for (int i = 0; i < size; i++) {
					souceInfor.put(ltmp.get(i), tmpHeadID + ","
							+ tmpbodys2[index2].getPrimaryKey());
				}
				index2++;
			}
			index++;
	...
	private void reWriteSouceInfor(Map<String, String> souceInfor, PlanBVO body)
			throws BusinessException {
		String sql = " update " + body.getTableName()
				+ " set cnextbillid = ? ,cnextbillbid = ? ,irowstatus = "
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业）计划弃审时校验 2011-1-29下午12:50:23
	 * @param spocorp
	 * @param sLogCorp
	 * @param uLogdate
	 * @param billvo
	 * @throws BusinessException
	 */
	public void checkPlanOnUnApprove(String spocorp, String sLogCorp,
			UFDate uLogdate, AggregatedValueObject billvo)
			throws BusinessException {
		...
		boolean flag = false;
		for (PlanBVO body : bodys) {
			if (PuPubVO.getInteger_NullAs(body.getIrowstatus(),
					HgPubTool.INTEGER_ZERO_VALUE).intValue() == HgPubConst.PLAN_ROW_STATUS_COMMIT) {// 存在已提交行
		...
		if (flag) {
			...
			String sql = "select count(*) from  po_praybill_b where csourcebillid = '"
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业）计划弃审时回写 2011-1-29下午01:48:13
	 * @param billvo
	 * @throws BusinessException
	 */
	public void reWriteNextBillOnUnapprove(HYBillVO billvo)
			throws BusinessException {
		if (billvo == null)
			return;
		PlanVO head = (PlanVO) billvo.getParentVO();
		if (head.getPk_billtype().equalsIgnoreCase(
				HgPubConst.PLAN_YEAR_BILLTYPE)) {
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业）计划删除时回写 2011-1-29下午01:48:13
	 * @param billvo
	 * @throws BusinessException
	 */
	public void reWriteNextBillOnDel(AggregatedValueObject billvo)
			throws BusinessException {
		if (billvo == null)
			return;
		PlanVO head = (PlanVO) billvo.getParentVO();
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业） 计划调整净需求 存入 调整明细 2011-4-3下午05:12:16
	 * @param items
	 * @param oldNum
	 * @param coprator
	 * @param dLogDate
	 * @throws BusinessException
	 */
	public void adjustPlanNetNum(PlanBVO[] items, Object useObj,
			String coprator, UFDate dLogDate) throws BusinessException {
		...
		for (PlanBVO item : items) {
			...
			tmp.setNupdatenum(PuPubVO.getUFDouble_NullAsZero(oldNum.get(item
					.getPrimaryKey())));
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业）查询用户角色权限 2011-4-6下午03:19:07
	 * @param tableName
	 * @param tableShowName
	 * @param pk_corp
	 * @param pk_user
	...
	public PactItemVO[] getPactItemsForPO(String corderid)
			throws BusinessException {
		...
		strbff
				.append(" from CT_TERM_BB4 where isnull(dr,0)=0 and pk_ct_manage = '"
						+ corderid + "'");
		Object o = getBaseDao().executeQuery(strbff.toString(),
				new BeanListProcessor(PactItemVO.class));
		if (o == null)
			return null;
		List ldata = (List) o;
		if (ldata.size() == 0)
	 ...
	 * 
	 * @author zhf
	 * @说明：（鹤岗矿业）保存合同条款页签 2012-2-23下午03:00:11
	 * @param pacts
	 * @return
	 * @throws BusinessException
	 */
	public Object savePactItemsForPO(PactItemVO[] pacts)
			throws BusinessException {
		...
		Collection c = getBaseDao().retrieveByClause(PactItemVO.class,
				" pk_ct_manage = '" + headid + "' and isnull(dr,0)=0");
		if (c == null || c.size() == 0)
			return null;
	...
	public void checkMonPlan(AggregatedValueObject billvo)
			throws BusinessException {
		if (billvo == null)
			return;
