  ...
  private void setBillStatus(int iChangeStatus, String sOperationName) {
    ...
    for (int i = 0; i < voSel.getChildrenVO().length; i++) {
      ...
      if (sOperationName.equals(nc.ui.ml.NCLangRes.getInstance().getStrByID(
        ...
        BillItemVO vo =(BillItemVO)voSel.getChildrenVO()[i];
        //add by zhw   2011-01-13  当调拨订单来源计划时   调拨订单关闭 回写 当前没有出库的数量到计划中
        String  billid = PuPubVO.getString_TrimZeroLenAsNull(vo.getCfirstbid());
        if(billid !=null){
        	UFDouble backnum =vo.getNnum().sub(vo.getNorderoutnum());
        	
        	try {
        		Class[] ParameterTypes = new Class[]{UFDouble.class,String.class};
        		Object[] ParameterValues = new Object[]{backnum,billid};
        		Object o=LongTimeTask.callRemoteService("scmpub","nc.bs.hg.scm.pub.HgScmPubBO", "reWriteToPlanFor5XEnd", ParameterTypes, ParameterValues, 2);
			} catch (Exception e) {
