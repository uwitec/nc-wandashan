...
//		setLeftListModel(m_leftModel);
		...
		for (int i=0;i<m_leftVOs.length;i++)
		 {
		   ...
		   String[] names = null;
		try {
			names = WdsWlPubTool.splitCode(getLeftData().getShowFieldName());
		...
		if(names!=null)
		   for(String name:names){
			   showName.append(WdsWlPubTool.getString_NullAsTrimZeroLen(m_leftVOs[i].getAttributeValue(name))+" ");
		   }
			...
			for (int i=0;i<m_rightVOs.length;i++)
			 {
			   ...
			   String[] names = null;
				...
				if(names!=null)
				   for(String name:names){
					   showName.append(WdsWlPubTool.getString_NullAsTrimZeroLen(m_rightVOs[i].getAttributeValue(name))+" ");
				   }
