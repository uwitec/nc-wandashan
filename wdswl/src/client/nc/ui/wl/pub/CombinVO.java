...
public class CombinVO {
	    ...
	    */
		 ...
		 * @param voCombinConds 条件字段数组（根据这个维度条件将两个数组合并为一个）
		 * @param types 求值类型 (要求值的字段类型 注册接口为IUFTypes)
		 * @param combinFields 求值字段（要进行求和合并的字段数组）
		 * @return
		 */
	 ...
	 * @param vos 报表集合
	 * @param vos1 报表集合
	 * @param xid  将vos1中的数据追加到vos中时 会将 vos1中属性名字 改为 属性名+xid
	 *             如果 xid为空 则不追加标示
	 */
...
public static ReportBaseVO[] addByContion2(ReportBaseVO[] vos,ReportBaseVO[] vos1,String[] voCombinConds,String xid)throws Exception{
		 ...
		 * 按某个为度 将符合条件的数据进行合并
		 * @author zhf
		 * @param accounts  要合并的报表集合
		 * @param num_condition_fields 合并的维度条件
		 * @param combinFields 合并的字段  只支持UFDouble
		 * @return
		 * @throws Exception 
		 */
