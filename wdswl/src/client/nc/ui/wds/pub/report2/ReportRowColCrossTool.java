...
public class ReportRowColCrossTool {
	...
	/**
	 * 交叉二维表 按交叉字段进行横向数据合计 没有实现用不了
	 * 
	 * @author mlr
	 * @说明：（鹤岗矿业） 2011-12-19上午09:56:24
	 * @param ui
	 */
	public static void onTotal(ReportBaseUI ui, int start) {
		if (ui == null)
			return;
		indexs.clear();
		ReportItem[] items = (ReportItem[]) ui.getReportBase().getBillModel()
				.getBodyItems();
		...
		for (int i = 0; i < indexs.size(); i++) {
			...
			ReportItem item = ReportPubTool.getItem("numˉ" + i, "合计",
					IBillItem.DECIMAL, 2, 80);
			// 构建合计的公式
			String st = "";
	...
	public static void onCross(ReportBaseUI ui, MemoryResultSetMetaData mrsmd,
		...
		drawCrossTable(ui, mrsmd, vc, risOrg, strows, new String[] {
				comCols.toString(), "&type" }, strCrsVals);
		// drawCrossTable(ui,strows,strCrsCols,strCrsVals);
