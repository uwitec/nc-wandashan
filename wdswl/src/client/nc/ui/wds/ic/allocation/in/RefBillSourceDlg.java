...
/**
 * 
 * @author Administrator *物流调拨入库(WDS9)参照-->erp调拨出库单
 */
public class RefBillSourceDlg extends WdsBillSourceDLG {
	...
	public String getHeadCondition() {
		...
		hsql.append("(");
		hsql
				.append("select distinct cgeneralhid from ic_general_b where isnull(ic_general_b.dr,0)=0");
		hsql.append(" and coalesce(noutnum,0)-coalesce("
				+ WdsWlPubConst.erp_allo_outnum_fieldname + ",0)>0");// 应入数量-转出数量>0
	...
	public String getBodyCondition() {
		// String sub = getTempTableUtil().getSubSql(inv_Pks);
		return " coalesce(body.noutnum,0)-coalesce(body."
				+ WdsWlPubConst.erp_allo_outnum_fieldname + ",0)>0" + // 应入数量-转出数量>0
