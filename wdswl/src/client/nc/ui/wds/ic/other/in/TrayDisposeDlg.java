...
public class TrayDisposeDlg extends nc.ui.pub.beans.UIDialog implements 
			ActionListener, ListSelectionListener,BillEditListener,BillEditListener2{
	...
	public TrayDisposeDlg(String m_billType, String m_operator,String m_pkcorp, 
			String m_nodeKey, InPubClientUI myClientUI,boolean isEdit) {
		super(myClientUI);
	...
	private void init() {
		...
		getbillListPanel().getBodyScrollPane("tb_general_b_b").addEditListener2(this);
		//加载表头数据
		loadHeadData();
	}
...
//			bbvo.setGebb_rowno(String.valueOf((i+1)*10));//行号
	...
	protected TbGeneralBVO getHeadBVO(int row){
		TbGeneralBVO vo  = (TbGeneralBVO)getbillListPanel().getHeadBillModel().getBodyValueRowVO(row, TbGeneralBVO.class.getName());
		return vo;
	}
	
	protected TbGeneralBBVO getBodyVO(int row){
		TbGeneralBBVO vo  = (TbGeneralBBVO)getbillListPanel().getBodyBillModel().getBodyValueRowVO(row, TbGeneralBBVO.class.getName());
		return vo;
	}
	...
	public void saveCurrentData(int row){
		...
		TbGeneralBBVO[] bvos = (TbGeneralBBVO[])getbillListPanel().getBodyBillModel().getBodyValueVOs(TbGeneralBBVO.class.getName());
	...
	public TbGeneralBVO getGenBVO(String crowno){
		...
		for(int i = 0 ;i < row; i++){
			Object o = getbillListPanel().getHeadBillModel().getValueAt(i, "geb_crowno");
			if(o.equals(crowno)){
				bvo = getHeadBVO(i);
			}
		}
	...
	public void valueChanged(ListSelectionEvent e) {
		if (e.getSource() == getbillListPanel().getHeadTable().getSelectionModel()) {
			//备份数据
			//重新加载表体数据
		}
	}
	...
	public boolean beforeEdit(BillEditEvent e) {
		...
		if("trayname".equals(key)){//托盘指定，增加托盘过滤，
			...
			UIRefPane ref = (UIRefPane)getbillListPanel().getBodyItem("trayname").getComponent();
			ref.getRefModel().addWherePart(" and bd_cargdoc_tray.cdt_invmandoc = '"+bvo.getPk_invmandoc()
					+"'  and  bd_cargdoc_tray.cdt_traystatus = 0 and bd_cargdoc_tray.pk_cargdoc = '"	+bvo.getPk_cargdoc()
		...
		if("ninassistnum".equalsIgnoreCase(key)){
			String cdt_id = PuPubVO.getString_TrimZeroLenAsNull(getbillListPanel().getBodyBillModel().getValueAt(row, "cdt_pk"));
			if(cdt_id == null){
				MessageDialog.showErrorDlg(this, "错误", "第 "+(row+1)+"行，请先选择托盘");
				return false;
			}
		}
		return true;
	...
	public void afterEdit(BillEditEvent e) {
		...
		if ("ninassistnum".equals(key)) {
			Object o = getbillListPanel().getBodyBillModel().getValueAt(row, "traymax");
			UFDouble traymax = PuPubVO.getUFDouble_NullAsZero(o.toString());// 托盘最大容积
			UFDouble ninassistnum = PuPubVO.getUFDouble_NullAsZero(e.getValue());//实入辅数量
			if(ninassistnum.sub(traymax).doubleValue()>0){
				getbillListPanel().getBodyBillModel().setValueAt(null, row, "ninassistnum");
				getbillListPanel().getBodyBillModel().setValueAt(null, row, "gebb_num");
				MessageDialog.showErrorDlg(this, "错误", "第 "+(row+1)+"行，托盘存放数量大于托盘最大容积！");
				return;
			}
		}
		if("trayname".equalsIgnoreCase(key)){
			...
			if(value.toUpperCase().contains("XN")){
				getbillListPanel().getBodyBillModel().setValueAt(100000000, row, "traymax");
			}else{
				getbillListPanel().getBodyBillModel().execLoadFormulaByKey("pk_invmandoc");
			}
		}
		saveCurrentData(getHeadCurrentRow());
	}
