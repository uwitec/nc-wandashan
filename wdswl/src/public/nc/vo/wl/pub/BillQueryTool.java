...
/**
 * 该工具主要用来 通过单据类型 单据id来查询上游单据或下游单据
 * 
 * @author mlr
 */
public class BillQueryTool {
	/**
	 * 返回查询来源单据的SQL 原理： 根据当前单据类型 billType 获得VoTable 通过VoTable 拼接一个主子表关联的 sql语句
	 * 根据billid 查询出 当前单据的 单据号 来源单据的单据类型 和单据id 来源单据类字段必须为 vlastbilltype
	 * 来源单据id字段必须为 vlastbillid 单据类型字段必须为 pk_billtype
	 * 
	...
	/**
	 * 功能描述:创建查询语句;在该类型的单据中查找某种类型单据的后续单据 输入参数:String billType,单据类型. 当前单据的 下游单据类型
	 * 返回值: SQL语句 异常处理: 日期:
	 * 
	 * @return java.lang.String
	 * @param billType
	...
	protected static String createSQL1(String billType) {
		...
		String hTableBillTypeField = paraVo.getBillType();// 单据类型 --> 对应votable
		// 的 def1 默认
		// pk_billtype
		String bTable = itemAttrVo.getVotable();
		String bFkField = itemAttrVo.getPkfield();
		String bTableSourceTypeField = paraVo.getLastBillType();// 上层单据类型-->对应votable
		// 的def2
		// 默认vlastbilltype
		String bTableSourceIDField = paraVo.getLastBillID();// 上层单据id
		// -->对应votable
		// 的def3
	...
	public static nc.vo.trade.billsource.LightBillVO[] getForwardBills(
			String curBillType, String curBillID, final String forwardBillType) {
		...
		try {
			...
			if (result.size() == 0)
				...
				return (LightBillVO[]) result.toArray(new LightBillVO[result
						.size()]);
		} catch (DbException e) {
	 ...
	 * curBillType :当前单据类型 2.String curBillID:当前单据ID 要想查出所有的来源 请自行采用递归算法查询
	 */
