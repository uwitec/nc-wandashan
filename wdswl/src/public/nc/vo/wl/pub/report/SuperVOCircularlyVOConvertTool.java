...
/**
 * SuperVO与CircularlyAccessibleValueObject转换工具
 * 报表调用平台行业TreeGather组件汇总树数据时调用
 * 注意：仅限报表使用，单据进行树汇总请使用该单据的VO
 * 作者：薛恩平
 * 创建日期：2006-4-20 13:05:07
 */
public class SuperVOCircularlyVOConvertTool {

	/**
	 * CircularlyAccessibleValueObject数组转为SuperVO数组
	 * 报表树汇总前调用
	 * 作者：薛恩平
	...
	/**
	 * SuperVO数组转为CircularlyAccessibleValueObject数组
	 * 报表树汇总后调用
	 * 作者：薛恩平
	 ...
	 * @return
	 */
	public static CircularlyAccessibleValueObject[] superVOTocircularlyVO(SuperVO[] supVOs) {
		...
		for (int i = 0; i < supVOs.length; i++) {
			...
			for (int j = 0; j < supAttrNames.length; j++) {
				cirVOs[i].setAttributeValue(supAttrNames[j], supVOs[i].getAttributeValue(supAttrNames[j]));
			}
		}
