...
//		sbSql = sbSql.append(")");
	 ...
	 * @param trayInfor
	 */
	public static void setDatasToPanelForOutBill(TbOutgeneralBVO body,
			BillCardPanel card) throws BusinessException {
		if(body == null)
			return;
		List<TbOutgeneralTVO> ltray = body.getTrayInfor();
		if(ltray== null ||ltray.size() == 0){
			throw new BusinessException("托盘信息为空");
		}
//		String key = null;
		int row = 0;
//		StringBuffer msg = new StringBuffer();
		java.util.List<String> ltmpbatch = new ArrayList<String>();
//		for (TbOutgeneralBVO body : bodys) {
			card.setBodyValueAt(null, row, "noutnum");
			card.setBodyValueAt(null, row, "noutassistnum");
			card.setBodyValueAt(null, row, "vbatchcode");
//			String key = body.getCinvbasid();
			// 调用接口查询缓存表中的实出主辅数量
...
//			TbOutgeneralTVO[][] os = (TbOutgeneralTVO[][]) SplitBillVOs
//					.getSplitVOs(ltray.toArray(new TbOutgeneralTVO[0]),
//							WdsWlPubConst.out_split_names);
//			if (os == null)
//				continue;
//			if (os.length > 1){// 多批次时{
//				
////				continue;
////				msg.append("第"+body.getCrowno()+"行货品自动拣货失败！满足本次出库数量的货品存在多批次，请手工指定");
//			}
//				continue;
...
//			// 金额
//			card.setBodyValueAt(nmny, row, "nmny");
//			// 来源批次
//			card.setBodyValueAt(ltray.get(0).getLvbatchcode(), row,
//					"lvbatchcode");
