  /***************************************************************\
  *     The skeleton of this class is generated by an automatic *
  * code generator for NC product. It is based on Velocity.     *
  \***************************************************************/
      	package nc.vo.wds.tranprice.cardoc;
   	
	import java.util.ArrayList;
	import nc.vo.pub.*;
	import nc.vo.pub.lang.*;
	
/**
 * 
 *
 * <p>
 *     车辆档案子表
 * </p>
 *
 * 创建日期:2011-5-19
 * @author mlr
 * @version Your Project 1.0
 */
     public class CardocBVO extends SuperVO {
           
             public String vdriverqq;
             public String reserve5;
             public String vdef4;
             public String vdef18;
             public String vdriveraddr;
             public Integer dr;
             public String vdef16;
             public String vdef17;
             public String vdef12;
             public String vdrivername;
             public UFDate reserve12;
             public UFDateTime ts;
             public String pk_defdoc11;
             public UFDouble reserve6;
             public String pk_defdoc10;
             public UFBoolean reserve14;
             public String vdef19;
             public String pk_defdoc6;
             public String vdrivertye;
             public UFDouble reserve10;
             public String pk_defdoc20;
             public String pk_wds_cardoc_b;
             public String vdef3;
             public String vdef13;
             public String pk_defdoc3;
             public String pk_wds_cardoc_h;
             public String pk_defdoc12;
             public String reserve1;
             public UFBoolean reserve16;
             public String pk_defdoc2;
             public String vdriverphone1;
             public UFBoolean reserve15;
             public String pk_defdoc7;
             public String pk_defdoc17;
             public String reserve2;
             public String vdriverfax;
             public String vdef10;
             public String vdef5;
             public String pk_defdoc15;
             public String vdef11;
             public String reserve4;
             public String pk_defdoc16;
             public String vdef7;
             public String pk_defdoc13;
             public String vdef20;
             public String vdef2;
             public UFDate reserve11;
             public String vmemo;
             public String pk_defdoc18;
             public String vdef1;
             public String pk_defdoc9;
             public String reserve3;
             public String pk_defdoc1;
             public String vdriverphone2;
             public String pk_defdoc19;
             public String vdef14;
             public String pk_defdoc4;
             public UFDate reserve13;
             public String pk_defdoc5;
             public String vdriveremail;
             public UFDouble reserve8;
             public UFDouble reserve9;
             public String vdef9;
             public String vdef8;
             public String pk_defdoc8;
             public UFDouble reserve7;
             public String vdef6;
             public String vdef15;
             public String pk_defdoc14;
            
             public static final String  VDRIVERQQ="vdriverqq";   
             public static final String  RESERVE5="reserve5";   
             public static final String  VDEF4="vdef4";   
             public static final String  VDEF18="vdef18";   
             public static final String  VDRIVERADDR="vdriveraddr";   
             public static final String  DR="dr";   
             public static final String  VDEF16="vdef16";   
             public static final String  VDEF17="vdef17";   
             public static final String  VDEF12="vdef12";   
             public static final String  VDRIVERNAME="vdrivername";   
             public static final String  RESERVE12="reserve12";   
             public static final String  TS="ts";   
             public static final String  PK_DEFDOC11="pk_defdoc11";   
             public static final String  RESERVE6="reserve6";   
             public static final String  PK_DEFDOC10="pk_defdoc10";   
             public static final String  RESERVE14="reserve14";   
             public static final String  VDEF19="vdef19";   
             public static final String  PK_DEFDOC6="pk_defdoc6";   
             public static final String  VDRIVERTYE="vdrivertye";   
             public static final String  RESERVE10="reserve10";   
             public static final String  PK_DEFDOC20="pk_defdoc20";   
             public static final String  PK_WDS_CARDOC_B="pk_wds_cardoc_b";   
             public static final String  VDEF3="vdef3";   
             public static final String  VDEF13="vdef13";   
             public static final String  PK_DEFDOC3="pk_defdoc3";   
             public static final String  PK_WDS_CARDOC_H="pk_wds_cardoc_h";   
             public static final String  PK_DEFDOC12="pk_defdoc12";   
             public static final String  RESERVE1="reserve1";   
             public static final String  RESERVE16="reserve16";   
             public static final String  PK_DEFDOC2="pk_defdoc2";   
             public static final String  VDRIVERPHONE1="vdriverphone1";   
             public static final String  RESERVE15="reserve15";   
             public static final String  PK_DEFDOC7="pk_defdoc7";   
             public static final String  PK_DEFDOC17="pk_defdoc17";   
             public static final String  RESERVE2="reserve2";   
             public static final String  VDRIVERFAX="vdriverfax";   
             public static final String  VDEF10="vdef10";   
             public static final String  VDEF5="vdef5";   
             public static final String  PK_DEFDOC15="pk_defdoc15";   
             public static final String  VDEF11="vdef11";   
             public static final String  RESERVE4="reserve4";   
             public static final String  PK_DEFDOC16="pk_defdoc16";   
             public static final String  VDEF7="vdef7";   
             public static final String  PK_DEFDOC13="pk_defdoc13";   
             public static final String  VDEF20="vdef20";   
             public static final String  VDEF2="vdef2";   
             public static final String  RESERVE11="reserve11";   
             public static final String  VMEMO="vmemo";   
             public static final String  PK_DEFDOC18="pk_defdoc18";   
             public static final String  VDEF1="vdef1";   
             public static final String  PK_DEFDOC9="pk_defdoc9";   
             public static final String  RESERVE3="reserve3";   
             public static final String  PK_DEFDOC1="pk_defdoc1";   
             public static final String  VDRIVERPHONE2="vdriverphone2";   
             public static final String  PK_DEFDOC19="pk_defdoc19";   
             public static final String  VDEF14="vdef14";   
             public static final String  PK_DEFDOC4="pk_defdoc4";   
             public static final String  RESERVE13="reserve13";   
             public static final String  PK_DEFDOC5="pk_defdoc5";   
             public static final String  VDRIVEREMAIL="vdriveremail";   
             public static final String  RESERVE8="reserve8";   
             public static final String  RESERVE9="reserve9";   
             public static final String  VDEF9="vdef9";   
             public static final String  VDEF8="vdef8";   
             public static final String  PK_DEFDOC8="pk_defdoc8";   
             public static final String  RESERVE7="reserve7";   
             public static final String  VDEF6="vdef6";   
             public static final String  VDEF15="vdef15";   
             public static final String  PK_DEFDOC14="pk_defdoc14";   
      
    
        /**
	   * 属性vdriverqq的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriverqq() {
		 return vdriverqq;
	  }   
	  
     /**
	   * 属性vdriverqq的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriverqq String
	   */
	public void setVdriverqq(String newVdriverqq) {
		
		vdriverqq = newVdriverqq;
	 } 	  
       
        /**
	   * 属性reserve5的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getReserve5() {
		 return reserve5;
	  }   
	  
     /**
	   * 属性reserve5的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve5 String
	   */
	public void setReserve5(String newReserve5) {
		
		reserve5 = newReserve5;
	 } 	  
       
        /**
	   * 属性vdef4的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef4() {
		 return vdef4;
	  }   
	  
     /**
	   * 属性vdef4的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef4 String
	   */
	public void setVdef4(String newVdef4) {
		
		vdef4 = newVdef4;
	 } 	  
       
        /**
	   * 属性vdef18的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef18() {
		 return vdef18;
	  }   
	  
     /**
	   * 属性vdef18的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef18 String
	   */
	public void setVdef18(String newVdef18) {
		
		vdef18 = newVdef18;
	 } 	  
       
        /**
	   * 属性vdriveraddr的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriveraddr() {
		 return vdriveraddr;
	  }   
	  
     /**
	   * 属性vdriveraddr的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriveraddr String
	   */
	public void setVdriveraddr(String newVdriveraddr) {
		
		vdriveraddr = newVdriveraddr;
	 } 	  
       
        /**
	   * 属性dr的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return Integer
	   */
	 public Integer getDr() {
		 return dr;
	  }   
	  
     /**
	   * 属性dr的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newDr Integer
	   */
	public void setDr(Integer newDr) {
		
		dr = newDr;
	 } 	  
       
        /**
	   * 属性vdef16的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef16() {
		 return vdef16;
	  }   
	  
     /**
	   * 属性vdef16的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef16 String
	   */
	public void setVdef16(String newVdef16) {
		
		vdef16 = newVdef16;
	 } 	  
       
        /**
	   * 属性vdef17的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef17() {
		 return vdef17;
	  }   
	  
     /**
	   * 属性vdef17的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef17 String
	   */
	public void setVdef17(String newVdef17) {
		
		vdef17 = newVdef17;
	 } 	  
       
        /**
	   * 属性vdef12的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef12() {
		 return vdef12;
	  }   
	  
     /**
	   * 属性vdef12的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef12 String
	   */
	public void setVdef12(String newVdef12) {
		
		vdef12 = newVdef12;
	 } 	  
       
        /**
	   * 属性vdrivername的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdrivername() {
		 return vdrivername;
	  }   
	  
     /**
	   * 属性vdrivername的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdrivername String
	   */
	public void setVdrivername(String newVdrivername) {
		
		vdrivername = newVdrivername;
	 } 	  
       
        /**
	   * 属性reserve12的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDate
	   */
	 public UFDate getReserve12() {
		 return reserve12;
	  }   
	  
     /**
	   * 属性reserve12的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve12 UFDate
	   */
	public void setReserve12(UFDate newReserve12) {
		
		reserve12 = newReserve12;
	 } 	  
       
        /**
	   * 属性ts的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDateTime
	   */
	 public UFDateTime getTs() {
		 return ts;
	  }   
	  
     /**
	   * 属性ts的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newTs UFDateTime
	   */
	public void setTs(UFDateTime newTs) {
		
		ts = newTs;
	 } 	  
       
        /**
	   * 属性pk_defdoc11的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc11() {
		 return pk_defdoc11;
	  }   
	  
     /**
	   * 属性pk_defdoc11的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc11 String
	   */
	public void setPk_defdoc11(String newPk_defdoc11) {
		
		pk_defdoc11 = newPk_defdoc11;
	 } 	  
       
        /**
	   * 属性reserve6的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDouble
	   */
	 public UFDouble getReserve6() {
		 return reserve6;
	  }   
	  
     /**
	   * 属性reserve6的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve6 UFDouble
	   */
	public void setReserve6(UFDouble newReserve6) {
		
		reserve6 = newReserve6;
	 } 	  
       
        /**
	   * 属性pk_defdoc10的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc10() {
		 return pk_defdoc10;
	  }   
	  
     /**
	   * 属性pk_defdoc10的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc10 String
	   */
	public void setPk_defdoc10(String newPk_defdoc10) {
		
		pk_defdoc10 = newPk_defdoc10;
	 } 	  
       
        /**
	   * 属性reserve14的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFBoolean
	   */
	 public UFBoolean getReserve14() {
		 return reserve14;
	  }   
	  
     /**
	   * 属性reserve14的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve14 UFBoolean
	   */
	public void setReserve14(UFBoolean newReserve14) {
		
		reserve14 = newReserve14;
	 } 	  
       
        /**
	   * 属性vdef19的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef19() {
		 return vdef19;
	  }   
	  
     /**
	   * 属性vdef19的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef19 String
	   */
	public void setVdef19(String newVdef19) {
		
		vdef19 = newVdef19;
	 } 	  
       
        /**
	   * 属性pk_defdoc6的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc6() {
		 return pk_defdoc6;
	  }   
	  
     /**
	   * 属性pk_defdoc6的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc6 String
	   */
	public void setPk_defdoc6(String newPk_defdoc6) {
		
		pk_defdoc6 = newPk_defdoc6;
	 } 	  
       
        /**
	   * 属性vdrivertye的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdrivertye() {
		 return vdrivertye;
	  }   
	  
     /**
	   * 属性vdrivertye的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdrivertye String
	   */
	public void setVdrivertye(String newVdrivertye) {
		
		vdrivertye = newVdrivertye;
	 } 	  
       
        /**
	   * 属性reserve10的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDouble
	   */
	 public UFDouble getReserve10() {
		 return reserve10;
	  }   
	  
     /**
	   * 属性reserve10的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve10 UFDouble
	   */
	public void setReserve10(UFDouble newReserve10) {
		
		reserve10 = newReserve10;
	 } 	  
       
        /**
	   * 属性pk_defdoc20的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc20() {
		 return pk_defdoc20;
	  }   
	  
     /**
	   * 属性pk_defdoc20的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc20 String
	   */
	public void setPk_defdoc20(String newPk_defdoc20) {
		
		pk_defdoc20 = newPk_defdoc20;
	 } 	  
       
        /**
	   * 属性pk_wds_cardoc_b的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_wds_cardoc_b() {
		 return pk_wds_cardoc_b;
	  }   
	  
     /**
	   * 属性pk_wds_cardoc_b的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_wds_cardoc_b String
	   */
	public void setPk_wds_cardoc_b(String newPk_wds_cardoc_b) {
		
		pk_wds_cardoc_b = newPk_wds_cardoc_b;
	 } 	  
       
        /**
	   * 属性vdef3的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef3() {
		 return vdef3;
	  }   
	  
     /**
	   * 属性vdef3的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef3 String
	   */
	public void setVdef3(String newVdef3) {
		
		vdef3 = newVdef3;
	 } 	  
       
        /**
	   * 属性vdef13的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef13() {
		 return vdef13;
	  }   
	  
     /**
	   * 属性vdef13的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef13 String
	   */
	public void setVdef13(String newVdef13) {
		
		vdef13 = newVdef13;
	 } 	  
       
        /**
	   * 属性pk_defdoc3的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc3() {
		 return pk_defdoc3;
	  }   
	  
     /**
	   * 属性pk_defdoc3的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc3 String
	   */
	public void setPk_defdoc3(String newPk_defdoc3) {
		
		pk_defdoc3 = newPk_defdoc3;
	 } 	  
       
        /**
	   * 属性pk_wds_cardoc_h的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_wds_cardoc_h() {
		 return pk_wds_cardoc_h;
	  }   
	  
     /**
	   * 属性pk_wds_cardoc_h的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_wds_cardoc_h String
	   */
	public void setPk_wds_cardoc_h(String newPk_wds_cardoc_h) {
		
		pk_wds_cardoc_h = newPk_wds_cardoc_h;
	 } 	  
       
        /**
	   * 属性pk_defdoc12的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc12() {
		 return pk_defdoc12;
	  }   
	  
     /**
	   * 属性pk_defdoc12的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc12 String
	   */
	public void setPk_defdoc12(String newPk_defdoc12) {
		
		pk_defdoc12 = newPk_defdoc12;
	 } 	  
       
        /**
	   * 属性reserve1的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getReserve1() {
		 return reserve1;
	  }   
	  
     /**
	   * 属性reserve1的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve1 String
	   */
	public void setReserve1(String newReserve1) {
		
		reserve1 = newReserve1;
	 } 	  
       
        /**
	   * 属性reserve16的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFBoolean
	   */
	 public UFBoolean getReserve16() {
		 return reserve16;
	  }   
	  
     /**
	   * 属性reserve16的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve16 UFBoolean
	   */
	public void setReserve16(UFBoolean newReserve16) {
		
		reserve16 = newReserve16;
	 } 	  
       
        /**
	   * 属性pk_defdoc2的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc2() {
		 return pk_defdoc2;
	  }   
	  
     /**
	   * 属性pk_defdoc2的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc2 String
	   */
	public void setPk_defdoc2(String newPk_defdoc2) {
		
		pk_defdoc2 = newPk_defdoc2;
	 } 	  
       
        /**
	   * 属性vdriverphone1的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriverphone1() {
		 return vdriverphone1;
	  }   
	  
     /**
	   * 属性vdriverphone1的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriverphone1 String
	   */
	public void setVdriverphone1(String newVdriverphone1) {
		
		vdriverphone1 = newVdriverphone1;
	 } 	  
       
        /**
	   * 属性reserve15的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFBoolean
	   */
	 public UFBoolean getReserve15() {
		 return reserve15;
	  }   
	  
     /**
	   * 属性reserve15的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve15 UFBoolean
	   */
	public void setReserve15(UFBoolean newReserve15) {
		
		reserve15 = newReserve15;
	 } 	  
       
        /**
	   * 属性pk_defdoc7的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc7() {
		 return pk_defdoc7;
	  }   
	  
     /**
	   * 属性pk_defdoc7的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc7 String
	   */
	public void setPk_defdoc7(String newPk_defdoc7) {
		
		pk_defdoc7 = newPk_defdoc7;
	 } 	  
       
        /**
	   * 属性pk_defdoc17的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc17() {
		 return pk_defdoc17;
	  }   
	  
     /**
	   * 属性pk_defdoc17的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc17 String
	   */
	public void setPk_defdoc17(String newPk_defdoc17) {
		
		pk_defdoc17 = newPk_defdoc17;
	 } 	  
       
        /**
	   * 属性reserve2的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getReserve2() {
		 return reserve2;
	  }   
	  
     /**
	   * 属性reserve2的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve2 String
	   */
	public void setReserve2(String newReserve2) {
		
		reserve2 = newReserve2;
	 } 	  
       
        /**
	   * 属性vdriverfax的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriverfax() {
		 return vdriverfax;
	  }   
	  
     /**
	   * 属性vdriverfax的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriverfax String
	   */
	public void setVdriverfax(String newVdriverfax) {
		
		vdriverfax = newVdriverfax;
	 } 	  
       
        /**
	   * 属性vdef10的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef10() {
		 return vdef10;
	  }   
	  
     /**
	   * 属性vdef10的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef10 String
	   */
	public void setVdef10(String newVdef10) {
		
		vdef10 = newVdef10;
	 } 	  
       
        /**
	   * 属性vdef5的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef5() {
		 return vdef5;
	  }   
	  
     /**
	   * 属性vdef5的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef5 String
	   */
	public void setVdef5(String newVdef5) {
		
		vdef5 = newVdef5;
	 } 	  
       
        /**
	   * 属性pk_defdoc15的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc15() {
		 return pk_defdoc15;
	  }   
	  
     /**
	   * 属性pk_defdoc15的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc15 String
	   */
	public void setPk_defdoc15(String newPk_defdoc15) {
		
		pk_defdoc15 = newPk_defdoc15;
	 } 	  
       
        /**
	   * 属性vdef11的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef11() {
		 return vdef11;
	  }   
	  
     /**
	   * 属性vdef11的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef11 String
	   */
	public void setVdef11(String newVdef11) {
		
		vdef11 = newVdef11;
	 } 	  
       
        /**
	   * 属性reserve4的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getReserve4() {
		 return reserve4;
	  }   
	  
     /**
	   * 属性reserve4的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve4 String
	   */
	public void setReserve4(String newReserve4) {
		
		reserve4 = newReserve4;
	 } 	  
       
        /**
	   * 属性pk_defdoc16的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc16() {
		 return pk_defdoc16;
	  }   
	  
     /**
	   * 属性pk_defdoc16的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc16 String
	   */
	public void setPk_defdoc16(String newPk_defdoc16) {
		
		pk_defdoc16 = newPk_defdoc16;
	 } 	  
       
        /**
	   * 属性vdef7的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef7() {
		 return vdef7;
	  }   
	  
     /**
	   * 属性vdef7的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef7 String
	   */
	public void setVdef7(String newVdef7) {
		
		vdef7 = newVdef7;
	 } 	  
       
        /**
	   * 属性pk_defdoc13的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc13() {
		 return pk_defdoc13;
	  }   
	  
     /**
	   * 属性pk_defdoc13的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc13 String
	   */
	public void setPk_defdoc13(String newPk_defdoc13) {
		
		pk_defdoc13 = newPk_defdoc13;
	 } 	  
       
        /**
	   * 属性vdef20的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef20() {
		 return vdef20;
	  }   
	  
     /**
	   * 属性vdef20的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef20 String
	   */
	public void setVdef20(String newVdef20) {
		
		vdef20 = newVdef20;
	 } 	  
       
        /**
	   * 属性vdef2的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef2() {
		 return vdef2;
	  }   
	  
     /**
	   * 属性vdef2的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef2 String
	   */
	public void setVdef2(String newVdef2) {
		
		vdef2 = newVdef2;
	 } 	  
       
        /**
	   * 属性reserve11的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDate
	   */
	 public UFDate getReserve11() {
		 return reserve11;
	  }   
	  
     /**
	   * 属性reserve11的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve11 UFDate
	   */
	public void setReserve11(UFDate newReserve11) {
		
		reserve11 = newReserve11;
	 } 	  
       
        /**
	   * 属性vmemo的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVmemo() {
		 return vmemo;
	  }   
	  
     /**
	   * 属性vmemo的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVmemo String
	   */
	public void setVmemo(String newVmemo) {
		
		vmemo = newVmemo;
	 } 	  
       
        /**
	   * 属性pk_defdoc18的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc18() {
		 return pk_defdoc18;
	  }   
	  
     /**
	   * 属性pk_defdoc18的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc18 String
	   */
	public void setPk_defdoc18(String newPk_defdoc18) {
		
		pk_defdoc18 = newPk_defdoc18;
	 } 	  
       
        /**
	   * 属性vdef1的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef1() {
		 return vdef1;
	  }   
	  
     /**
	   * 属性vdef1的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef1 String
	   */
	public void setVdef1(String newVdef1) {
		
		vdef1 = newVdef1;
	 } 	  
       
        /**
	   * 属性pk_defdoc9的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc9() {
		 return pk_defdoc9;
	  }   
	  
     /**
	   * 属性pk_defdoc9的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc9 String
	   */
	public void setPk_defdoc9(String newPk_defdoc9) {
		
		pk_defdoc9 = newPk_defdoc9;
	 } 	  
       
        /**
	   * 属性reserve3的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getReserve3() {
		 return reserve3;
	  }   
	  
     /**
	   * 属性reserve3的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve3 String
	   */
	public void setReserve3(String newReserve3) {
		
		reserve3 = newReserve3;
	 } 	  
       
        /**
	   * 属性pk_defdoc1的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc1() {
		 return pk_defdoc1;
	  }   
	  
     /**
	   * 属性pk_defdoc1的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc1 String
	   */
	public void setPk_defdoc1(String newPk_defdoc1) {
		
		pk_defdoc1 = newPk_defdoc1;
	 } 	  
       
        /**
	   * 属性vdriverphone2的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriverphone2() {
		 return vdriverphone2;
	  }   
	  
     /**
	   * 属性vdriverphone2的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriverphone2 String
	   */
	public void setVdriverphone2(String newVdriverphone2) {
		
		vdriverphone2 = newVdriverphone2;
	 } 	  
       
        /**
	   * 属性pk_defdoc19的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc19() {
		 return pk_defdoc19;
	  }   
	  
     /**
	   * 属性pk_defdoc19的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc19 String
	   */
	public void setPk_defdoc19(String newPk_defdoc19) {
		
		pk_defdoc19 = newPk_defdoc19;
	 } 	  
       
        /**
	   * 属性vdef14的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef14() {
		 return vdef14;
	  }   
	  
     /**
	   * 属性vdef14的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef14 String
	   */
	public void setVdef14(String newVdef14) {
		
		vdef14 = newVdef14;
	 } 	  
       
        /**
	   * 属性pk_defdoc4的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc4() {
		 return pk_defdoc4;
	  }   
	  
     /**
	   * 属性pk_defdoc4的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc4 String
	   */
	public void setPk_defdoc4(String newPk_defdoc4) {
		
		pk_defdoc4 = newPk_defdoc4;
	 } 	  
       
        /**
	   * 属性reserve13的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDate
	   */
	 public UFDate getReserve13() {
		 return reserve13;
	  }   
	  
     /**
	   * 属性reserve13的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve13 UFDate
	   */
	public void setReserve13(UFDate newReserve13) {
		
		reserve13 = newReserve13;
	 } 	  
       
        /**
	   * 属性pk_defdoc5的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc5() {
		 return pk_defdoc5;
	  }   
	  
     /**
	   * 属性pk_defdoc5的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc5 String
	   */
	public void setPk_defdoc5(String newPk_defdoc5) {
		
		pk_defdoc5 = newPk_defdoc5;
	 } 	  
       
        /**
	   * 属性vdriveremail的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdriveremail() {
		 return vdriveremail;
	  }   
	  
     /**
	   * 属性vdriveremail的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdriveremail String
	   */
	public void setVdriveremail(String newVdriveremail) {
		
		vdriveremail = newVdriveremail;
	 } 	  
       
        /**
	   * 属性reserve8的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDouble
	   */
	 public UFDouble getReserve8() {
		 return reserve8;
	  }   
	  
     /**
	   * 属性reserve8的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve8 UFDouble
	   */
	public void setReserve8(UFDouble newReserve8) {
		
		reserve8 = newReserve8;
	 } 	  
       
        /**
	   * 属性reserve9的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDouble
	   */
	 public UFDouble getReserve9() {
		 return reserve9;
	  }   
	  
     /**
	   * 属性reserve9的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve9 UFDouble
	   */
	public void setReserve9(UFDouble newReserve9) {
		
		reserve9 = newReserve9;
	 } 	  
       
        /**
	   * 属性vdef9的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef9() {
		 return vdef9;
	  }   
	  
     /**
	   * 属性vdef9的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef9 String
	   */
	public void setVdef9(String newVdef9) {
		
		vdef9 = newVdef9;
	 } 	  
       
        /**
	   * 属性vdef8的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef8() {
		 return vdef8;
	  }   
	  
     /**
	   * 属性vdef8的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef8 String
	   */
	public void setVdef8(String newVdef8) {
		
		vdef8 = newVdef8;
	 } 	  
       
        /**
	   * 属性pk_defdoc8的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc8() {
		 return pk_defdoc8;
	  }   
	  
     /**
	   * 属性pk_defdoc8的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc8 String
	   */
	public void setPk_defdoc8(String newPk_defdoc8) {
		
		pk_defdoc8 = newPk_defdoc8;
	 } 	  
       
        /**
	   * 属性reserve7的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return UFDouble
	   */
	 public UFDouble getReserve7() {
		 return reserve7;
	  }   
	  
     /**
	   * 属性reserve7的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newReserve7 UFDouble
	   */
	public void setReserve7(UFDouble newReserve7) {
		
		reserve7 = newReserve7;
	 } 	  
       
        /**
	   * 属性vdef6的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef6() {
		 return vdef6;
	  }   
	  
     /**
	   * 属性vdef6的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef6 String
	   */
	public void setVdef6(String newVdef6) {
		
		vdef6 = newVdef6;
	 } 	  
       
        /**
	   * 属性vdef15的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getVdef15() {
		 return vdef15;
	  }   
	  
     /**
	   * 属性vdef15的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newVdef15 String
	   */
	public void setVdef15(String newVdef15) {
		
		vdef15 = newVdef15;
	 } 	  
       
        /**
	   * 属性pk_defdoc14的Getter方法.
	   *
	   * 创建日期:2011-5-19
	   * @return String
	   */
	 public String getPk_defdoc14() {
		 return pk_defdoc14;
	  }   
	  
     /**
	   * 属性pk_defdoc14的Setter方法.
	   *
	   * 创建日期:2011-5-19
	   * @param newPk_defdoc14 String
	   */
	public void setPk_defdoc14(String newPk_defdoc14) {
		
		pk_defdoc14 = newPk_defdoc14;
	 } 	  
       
       
    /**
	  * 验证对象各属性之间的数据逻辑正确性.
	  *
	  * 创建日期:2011-5-19
	  * @exception nc.vo.pub.ValidationException 如果验证失败,抛出
	  * ValidationException,对错误进行解释.
	 */
	 public void validate() throws ValidationException {
	
	 	ArrayList errFields = new ArrayList(); // errFields record those null

                                                      // fields that cannot be null.
       		  // 检查是否为不允许空的字段赋了空值,你可能需要修改下面的提示信息:
	
	   		if (pk_wds_cardoc_b == null) {
			errFields.add(new String("pk_wds_cardoc_b"));
			  }	
	   	
	    StringBuffer message = new StringBuffer();
		message.append("下列字段不能为空:");
		if (errFields.size() > 0) {
		String[] temp = (String[]) errFields.toArray(new String[0]);
		message.append(temp[0]);
		for ( int i= 1; i < temp.length; i++ ) {
			message.append(",");
			message.append(temp[i]);
		}
		throw new NullFieldException(message.toString());
		}
	 }
			   
       
   	/**
	  * <p>取得父VO主键字段.
	  * <p>
	  * 创建日期:2011-5-19
	  * @return java.lang.String
	  */
	public java.lang.String getParentPKFieldName() {
	  	 
	 		return "pk_wds_cardoc_h";
	 	
	}   
    
    /**
	  * <p>取得表主键.
	  * <p>
	  * 创建日期:2011-5-19
	  * @return java.lang.String
	  */
	public java.lang.String getPKFieldName() {
	 	  return "pk_wds_cardoc_b";
	 	}
    
	/**
      * <p>返回表名称.
	  * <p>
	  * 创建日期:2011-5-19
	  * @return java.lang.String
	 */
	public java.lang.String getTableName() {
				
		return "wds_cardoc_b";
	}    
    
    /**
	  * 按照默认方式创建构造子.
	  *
	  * 创建日期:2011-5-19
	  */
	public CardocBVO() {
			
			   super();	
	  }    
    
            /**
	 * 使用主键进行初始化的构造子.
	 *
	 * 创建日期:2011-5-19
	 * @param newPk_wds_cardoc_b 主键值
	 */
	 public CardocBVO(String newPk_wds_cardoc_b) {
		
		// 为主键字段赋值:
		 pk_wds_cardoc_b = newPk_wds_cardoc_b;
	
    	}
    
     
     /**
	  * 返回对象标识,用来唯一定位对象.
	  *
	  * 创建日期:2011-5-19
	  * @return String
	  */
	   public String getPrimaryKey() {
				
		 return pk_wds_cardoc_b;
	   
	   }

     /**
	  * 设置对象标识,用来唯一定位对象.
	  *
	  * 创建日期:2011-5-19
	  * @param newPk_wds_cardoc_b  String    
	  */
	 public void setPrimaryKey(String newPk_wds_cardoc_b) {
				
				pk_wds_cardoc_b = newPk_wds_cardoc_b; 
				
	 } 
           
	  /**
       * 返回数值对象的显示名称.
	   *
	   * 创建日期:2011-5-19
	   * @return java.lang.String 返回数值对象的显示名称.
	   */
	 public String getEntityName() {
				
	   return "wds_cardoc_b"; 
				
	 } 
} 
