...
//				throw new BusinessException("未查询到该出库仓库的调拨虚拟欠发数据");
//			}else if(heads.length >1){
//				throw new BusinessException("查询到该出库仓库多条自由态调拨虚拟欠发数据");
//			}
...
//	}
		...
		if (writeHvos != null && writeHvos.length > 0) {
			for(Writeback4cHVO writeHead:writeHvos){
				...
				if (vbillstatus == IBillStatus.CHECKPASS) {
					throw new BusinessException("下游调拨出库台账回写已经审批完成,不能再弃审");
				}
				//创建调拨出库回传单 聚合vo
				...
				//如果调拨出库回传单 来源明细  都是本次弃审的 调拨出库单 将调拨出库回传单设置为整单删除
				...
				if(isDel){
					...
					//将 调拨出库回传单   表体2 来源明细为  本次弃审的调拨出库单的   记录   设置为删除状态
					//将  调拨出库回传单  表体1 汇总信息 对应的  来源明细新为删除态的记录  的汇总量  减掉  并设置vo状态为修改态
			...
			throw new BuildException("查询下游调拨出库回传单据异常：未查询到数据");
		}
	}
	 ...
	 * @param key  调拨订单主键
	 * @param head 物流调拨出库表头vo
	 * @param list 物流调拨出库表体vo
	 * @return
	 * @throws BusinessException
	 */
	public MultiBillVO getMutiBillvo(String key, TbOutgeneralHVO head,
	   ...
	   //创建调拨出库回传单 聚合vo
		...
		if (orderBods == null || orderBods.length == 0) {
			throw new BusinessException("查询调拨订单表体失败");
		}
		// 根据订单主键 查询调拨出库回传单 表头
		...
		if (writeHvos != null && writeHvos.length > 0) {
			...
			//查看是否已经审批
			if (vbillstatus == IBillStatus.CHECKPASS) {
				throw new BusinessException("下游调拨出库台账回写已经审批完成,不能再审核");
			}
			//查询出 调拨出库回传单表体数据
			String where = " isnull(dr,0)=0 and pk_wds_writeback4c_h = '"
					+ writeHvos[0].getPrimaryKey() + "'";
			//调拨出库 回传表体 1(按调拨订单 表体行id 汇总的信息)
			Writeback4cB1VO[] b1vos = (Writeback4cB1VO[]) getHypubBO()
					.queryByCondition(Writeback4cB1VO.class, where);
			//调拨出库 回传表体 2（物流调拨出库单表体明细）
			Writeback4cB2VO[] b2vos = (Writeback4cB2VO[]) getHypubBO()
			.queryByCondition(Writeback4cB2VO.class, where);
			int i=0;
			//设置  调拨出库 回传表体 2 初始行号
			...
			//物流调拨出库 表体 --->  调拨出库 回传表体 2  数据交换  设置vo状态为新增
			Writeback4cB2VO[] b2vosNew = getWriteBackB2vo(i,head, list);
			//将 新增  调拨出库 回传表体 2 数据  追加到 已存在的 调拨出库 回传表体 2 数据上 
	...
	public Writeback4cHVO getWrirteBackHvo(String csaleid, String vbillno) {
		...
		head.setCsaleid(csaleid);// 调拨订单主键
		head.setVbillno(vbillno);// 单据号
		head.setVbillstatus(IBillStatus.FREE);// 单据状态
		head.setPk_billtype(WdsWlPubConst.WDSX);// 单据类型
	...
	public Writeback4cB1VO[] getWriteBackB1vo(BillItemVO[] orderBodys) {
		...
		for (int i = 0; i < orderBodys.length; i++) {
			...
			b1vos[i].setCsaleid(orderBodys[i].getCbillid());// 调拨订单主键
			b1vos[i].setCorder_bid(orderBodys[i].getCbill_bid());// 调拨订单字表主键
			b1vos[i].setPk_invmandoc(orderBodys[i].getCtakeoutinvid());// 存货管理id
			b1vos[i].setPk_invbasdoc(orderBodys[i].getCinvbasid());// 存货基本id
		  //b1vos[i].setUnit(orderBodys[i].getCunitid());// 存货主计量单位
			b1vos[i].setAssunit(orderBodys[i].getCastunitid());// 存货辅计量单位
	...
	public void sumNums(MultiBillVO writeBillVO) {
		...
		if (b2vos == null || b2vos.length == 0) {
			...
			for (Writeback4cB2VO b2vo : b2vos) {
				...
				UFDouble noutassistnum = PuPubVO.getUFDouble_NullAsZero(b2vo
						.getNoutassistnum());
				//根据  订单表体 id   将调拨出库 回传 单 表体2 的明细  的 应出数量  和 实际出数量    汇总到  调拨出库 回传 单 表体1 
				for (Writeback4cB1VO b1vo : b1vos) {
					UFDouble nnumber = PuPubVO.getUFDouble_NullAsZero(b1vo
							.getNnumber());// 累计运单数量
					UFDouble npacknumber = PuPubVO.getUFDouble_NullAsZero(b1vo
