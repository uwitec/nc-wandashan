...
public class TpydBO {
	 ...
	 * 
	 * @作者：mlr
	 * @说明：完达山物流项目 对移动数据作最后校验,防止移出托盘出现负结存
	 ...
	 * 
	 * @作者：mlr
	 * @说明：完达山物流项目 对移入托盘的校验 同一个托盘，移入的总量 不能 超出当前托盘的容量
	 * 
	...
	private void valuteInCdt(SuperVO[] vos) throws Exception {
		...
		for (int i = 0; i < size1; i++) {
			rnum = PuPubVO.getUFDouble_NullAsZero(newVos1[i]
					.getAttributeValue("nmovenum"));
			rbnum = PuPubVO.getUFDouble_NullAsZero(newVos1[i]
					.getAttributeValue("nmoveassnum"));
			pk_trayout = PuPubVO.getString_TrimZeroLenAsNull(newVos1[i]
					.getAttributeValue("pk_trayout"));
	 ...
	 * @说明：完达山物流项目 
	 * 增加现存量
	 * @时间：2011-12-5下午06:37:28
	 * @param pk_trayout  要获取当前现存量的托盘
	 * @param rnum	增加数量
	 * @param rbnum 增加辅数量
	 * @throws Exception
	 */
	private void update(String pk_trayout, UFDouble rnum, UFDouble rbnum)
			throws Exception {
		UFDouble dnum = null;// 获取当前托盘的库存主数量
		UFDouble dbnum = null;// 获取当前托盘的库存辅数量
		StockInvOnHandVO svo = null;// 现存量vo(用于形成存货状态)
		...
		Object o1 = getBaseDAO()
				.executeQuery(
						"select wds_invbasdoc.tray_volume from wds_invbasdoc where isnull(wds_invbasdoc.dr,0)=0 and wds_invbasdoc.pk_invmandoc = '"
								+ svo.getPk_invmandoc() + "'",
						WdsPubResulSetProcesser.COLUMNPROCESSOR);
		UFDouble maxnum = PuPubVO.getUFDouble_NullAsZero(o1);

		if (dbnum.doubleValue() + rbnum.doubleValue() > maxnum.doubleValue()) {
			throw new BusinessException("托盘移入数量超出当前托盘的可移入量");
		} else {
			svo.setWhs_stocktonnage(dnum.add(rnum));
			svo.setWhs_stockpieces(dbnum.add(rbnum));
			if (svo.getWhs_stockpieces().doubleValue() == 0
					&& svo.getWhs_stocktonnage().doubleValue() == 0) {
				svo.setWhs_status(1);
				// 如果托盘存量为空的话，将货位托盘信息中,对应托盘的状态设为未占用状态
				String sql1 = "update bd_cargdoc_tray set cdt_traystatus="
						+ StockInvOnHandVO.stock_state_null
						+ " where isnull(dr,0)=0 and cdt_pk='" + pk_trayout
						+ "'";
				getBaseDAO().executeUpdate(sql1);
			}
	...
	private void valuteOutCdt(SuperVO[] vos) throws Exception {
		...
		if (newVos != null && newVos.length != 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				dnum = PuPubVO
						.getUFDouble_NullAsZero(svo.getWhs_stocktonnage());
				dbnum = PuPubVO
						.getUFDouble_NullAsZero(svo.getWhs_stockpieces());
				if (dnum.doubleValue() < rnum.doubleValue()
						|| dbnum.doubleValue() < rbnum.doubleValue()) {
					throw new Exception("编码为  ["
							+ newVos[i].getAttributeValue("outtraycode")
							+ "]的托盘移出数量超出当前托盘的存量");
				} else {
					...
					svo.setWhs_stocktonnage(dnum.sub(rnum));
					svo.setWhs_stockpieces(dbnum.sub(rbnum));
					if (svo.getWhs_stockpieces().doubleValue() == 0
							&& svo.getWhs_stocktonnage().doubleValue() == 0) {
						svo.setWhs_status(1);
						// 如果托盘存量为空的话，将货位托盘信息中,对应托盘的状态设为未占用状态
						String sql1 = "update bd_cargdoc_tray set cdt_traystatus="
								+ StockInvOnHandVO.stock_state_null
								+ " where isnull(dr,0)=0 and cdt_pk='"
								+ pk_trayout + "'";
						getBaseDAO().executeUpdate(sql1);
					}
					getStockBO().updateWarehousestock(svo);
