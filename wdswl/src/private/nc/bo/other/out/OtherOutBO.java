 ...
 * 
 * 业务控制说明：
 * 库存出库单  保存时  需要新增 托盘明细子子表数据  和  修改 托盘存量信息表数据 
 *             签字时  需要将托盘状态根据托盘存量进行 设置  0为空盘  1为有货  删除存量为0的托盘存量数据
 *             取消签字时  签字的逆操作
 *             删除时  删除单据  删除托盘明细子子表   回复托盘存量信息表的存量
 *             修改保存时  先做删除时的操作，然后同新增 * 
 * 
 */
public class OtherOutBO {
	 ...
	 * 
	 * @作者：zhf
	 * @说明：完达山物流项目 完达山物流项目 暂时未考虑数量修改后的回写   出库单回写
	 * 其他出库回写：发运订单（WDS3），采购取样(WDSC)
	...
	public void writeBack(MyBillVO newBillVo,int  iBdAction,boolean isNew)
		...
		sourcetype = PuPubVO.getString_TrimZeroLenAsNull(bodys[0].getCsourcetype());
		...
		if(iBdAction == IBDACTION.DELETE){
			for(TbOutgeneralBVO body:bodys){				
				if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS3)
						...
						numInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutnum()).multiply(-1));
						numassInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutassistnum()).multiply(-1)); 
				}
			}
			...
			if(isNew){
				for(TbOutgeneralBVO body:bodys){				
					if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS3)
							...
							sourcetype.equalsIgnoreCase("30")
							||sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS5)){									
						numInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutnum())); 
						numassInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutassistnum())); 
					...
					if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS3)
							...
							if(body.getStatus() == VOStatus.NEW){
								numInfor.put(body.getCsourcebillbid(), 
										PuPubVO.getUFDouble_NullAsZero(body.getNoutnum()));
								numassInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutassistnum())); 

							}else if(body.getStatus() == VOStatus.DELETED){
								numInfor.put(body.getCsourcebillbid(), 
										PuPubVO.getUFDouble_NullAsZero(body.getNoutnum()).multiply(-1));
								numassInfor.put(body.getCsourcebillbid(),PuPubVO.getUFDouble_NullAsZero(body.getNoutassistnum()).multiply(-1)); 
								...
								Object o =getBaseDAO().executeQuery(sql,para, WdsPubResulSetProcesser.ARRAYLISTPROCESSOR);
								if( o != null){
								...
								numInfor.put(body.getCsourcebillbid(), 
										PuPubVO.getUFDouble_NullAsZero(body.getNoutnum()).sub(noldoutnum));
								numassInfor.put(body.getCsourcebillbid(), 
										PuPubVO.getUFDouble_NullAsZero(body.getNoutassistnum()).sub(noutassistnum));
							}
					}
				}
			}
		...
		if(numInfor.size()>0){
			...
			if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS3)){
				...
				sql ="update so_saleorder_b set "+WdsWlPubConst.DM_SO_DEALNUM_FIELD_NAME+" = coalesce("+WdsWlPubConst.DM_SO_DEALNUM_FIELD_NAME+",0)+? " +
	...
	public void checkNoutNumByOrderNum(String sourcetype,String[] ids) throws BusinessException{
		...
		if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS3)){
			...
			"and pk_sendorder_b in "+getTempTableUtil().getSubSql(ids);
		}else if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDS5)){			
			sql = "select count(0) from wds_soorder_b where (coalesce(narrangnmu,0) - coalesce(noutnum,0)) < 0 " +
			"and pk_soorder_b in "+getTempTableUtil().getSubSql(ids);
		}else if(sourcetype.equalsIgnoreCase(WdsWlPubConst.WDSC)){
			...
			sql = "select count(0) from so_saleorder_b where (coalesce(nnumber,0) - coalesce("+WdsWlPubConst.DM_SO_DEALNUM_FIELD_NAME+",0)) < 0 " +
			"and corder_bid in "+getTempTableUtil().getSubSql(ids);
		}
		if(PuPubVO.getInteger_NullAs(getBaseDAO().executeQuery(sql, WdsPubResulSetProcesser.COLUMNPROCESSOR), -1)>0){
			throw new BusinessException("超发运订单出库，请核实实出数量");
		}
	}
	
	public void deleteOtherInforOnDelBill(List<TbOutgeneralTVO> ltray) throws BusinessException{
		//恢复托盘存量
	 ...
	 * @throws BusinessException
	 */
	public void backTrayInforOnDelBill(List<TbOutgeneralTVO> ltray) throws BusinessException{
		...
		for(TbOutgeneralTVO tray:ltray){
			whereSql = " pplpt_pk = '"+tray.getCdt_pk()+"' and pk_invbasdoc = '"+tray.getPk_invbasdoc()+"' and isnull(dr,0)=0 and whs_batchcode = '"+tray.getVbatchcode()+"'";
			List<StockInvOnHandVO> linv = (List<StockInvOnHandVO>)getBaseDAO().retrieveByClause(StockInvOnHandVO.class, whereSql);
			...
			trayInv.setWhs_stockpieces(PuPubVO.getUFDouble_NullAsZero(trayInv.getWhs_stockpieces()).add(PuPubVO.getUFDouble_NullAsZero(tray.getNoutassistnum())));
			trayInv.setWhs_stocktonnage(PuPubVO.getUFDouble_NullAsZero(trayInv.getWhs_stocktonnage()).add(PuPubVO.getUFDouble_NullAsZero(tray.getNoutnum())));
		...
		if(linvOnhand.size()>0)
			getBaseDAO().updateVOArray(linvOnhand.toArray(new StockInvOnHandVO[0]), WdsWlPubConsts.stockinvonhand_fieldnames);
