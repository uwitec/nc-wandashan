...
public class ClientUI extends nc.ui.ic.pub.bill.GeneralBillClientUI {
	...
	protected class ButtonManager201 extends
		...
		private boolean unButtonClicked(nc.ui.pub.ButtonObject bo) {
			...
			if (vo != null) {
				if (PuPubVO.getUFBoolean_NullAs(
						vo.getHeaderVO().getAttributeValue(
								HgPubConst.VUSERDEF[5]), UFBoolean.FALSE)
						.booleanValue()) {
		...
		private void onCheck() {
			...
			try {
				...
				if (HgPubConst.purchaseIn_in == HgPubTool
						.getPurchaseInBusiStatus(vo)) {
					showErrorMsg("该单据已经入库,不能验收");
					return;
				}
				// GeneralBillItemVO[] items =
				// (GeneralBillItemVO[])vo.getChildrenVO();
				...
				//				
				// if(isPact){
				// showErrorMsg("该单据已经合同匹配,不可验收");
				// return;
				// }

				if (HgPubConst.purchaseIn_ok == HgPubTool
						.getPurchaseInBusiStatus(vo)
						|| HgPubConst.purchaseIn_no == HgPubTool
								.getPurchaseInBusiStatus(vo)
						|| HgPubConst.purchaseIn_nopact == HgPubTool
								.getPurchaseInBusiStatus(vo)) {
					showErrorMsg("该单据已经验收，不可重复验收");
					return;
				}
				vo.getHeaderVO().setAttributeValue("cuserid",
				...
				if (HgPubConst.purchaseIn_ok == HgPubTool
						.getPurchaseInBusiStatus(billVO)) {
					showHintMessage("验收合格");
				} else if (HgPubConst.purchaseIn_no == HgPubTool
						.getPurchaseInBusiStatus(billVO)) {
				...
				showErrorMessage(HgPubTool.getString_NullAsTrimZeroLen(e
						.getMessage()));
				e.printStackTrace();
			}
		}
		...
		private CkeckDLG getCkeckDLG() throws Exception {
			if (m_CkeckDLG == null) {
				m_CkeckDLG = new CkeckDLG(this.getClientUI(),
		...
		private void setDataToUI(GeneralBillVO billVO) {
			...
			GeneralBillItemVO[] items = (GeneralBillItemVO[]) billVO
					.getChildrenVO();
			int len = items.length;
			if (BillMode.List == getM_iCurPanel()) {
				int selrow = getBillListPanel().getHeadTable().getSelectedRow();
				for (int i = 0; i < 6; i++) {
					setListHeadItem(head
							.getAttributeValue(HgPubConst.VUSERDEF[i]), selrow,
							HgPubConst.VUSERDEF[i]);
				}
				setListHeadItem(head.getTs(), selrow, "ts");
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[0]),selrow,HgPubConst.VUSERDEF[0]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[1]),selrow,HgPubConst.VUSERDEF[1]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[2]),selrow,HgPubConst.VUSERDEF[2]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[3]),selrow,HgPubConst.VUSERDEF[3]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[4]),selrow,HgPubConst.VUSERDEF[4]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[5]),selrow,HgPubConst.VUSERDEF[5]);

				String[] aryAssistunit = new String[] {
						"fis->" + HgPubConst.VUSERDEF[0],
						"dealid->" + HgPubConst.VUSERDEF[3],
						"dealname->getColValue(sm_user,user_name,cuserid,"
								+ HgPubConst.VUSERDEF[3] + ")",
						"fisclose->" + HgPubConst.VUSERDEF[5], };

				getBillListPanel().getHeadBillModel().execFormulas(selrow,
						aryAssistunit);
				for (int i = 0; i < len; i++) {
					getBillListPanel()
							.getBodyBillModel()
							.setValueAt(
									PuPubVO
											.getUFDouble_NullAsZero(items[i]
													.getAttributeValue(HgPubConst.NUM_DEF_QUA)),
				...
				// setCardHeadItem(HgPubConst.VUSERDEF[0],head.getAttributeValue(HgPubConst.VUSERDEF[0]));
				// setCardHeadItem(HgPubConst.VUSERDEF[1],head.getAttributeValue(HgPubConst.VUSERDEF[1]));
				// setCardHeadItem(HgPubConst.VUSERDEF[2],head.getAttributeValue(HgPubConst.VUSERDEF[2]));
				// setCardHeadItem(HgPubConst.VUSERDEF[3],head.getAttributeValue(HgPubConst.VUSERDEF[3]));
				// setCardHeadItem(HgPubConst.VUSERDEF[4],head.getAttributeValue(HgPubConst.VUSERDEF[4]));
				// setCardHeadItem(HgPubConst.VUSERDEF[5],head.getAttributeValue(HgPubConst.VUSERDEF[5]));
		...
		protected void onSwitch() {
			...
			if (BillMode.List == getM_iCurPanel()) {
				...
				String[] aryAssistunit = new String[] {
						...
						"updatename->getColValue(sm_user,user_name,cuserid,"
								+ HgPubConst.VUSERDEF[7] + ")" };
				getBillListPanel().getHeadBillModel().execFormulas(selrow,
						aryAssistunit);
			} else {
				String[] aryAssistunit = new String[] {
		...
		private void showErrorMsg(String msg) {
			MessageDialog.showErrorDlg(this.getClientUI(), "",
					msg == null ? "操作出现异常，请重新操作" : msg);
		}
		...
		private void onUNCheck() {
			...
			try {
				...
				GeneralBillItemVO[] items = (GeneralBillItemVO[]) vo
						.getChildrenVO();
				int len = items.length;
				...
				boolean isPact = false;
				for (GeneralBillItemVO item : items) {
					if (PuPubVO.getString_TrimZeroLenAsNull(item
							.getCsourcetype()) != null)
						isPact = true;
				}
				String self = HgPubTool
						.getString_NullAsTrimZeroLen(getBillCardPanel()
								.getHeadItem(HgPubConst.F_IS_SELF)
								.getValueObject());
				// 如果是自制 不进行合同匹配校验
				...
				Object o = LongTimeTask.callRemoteService("ic",
						"nc.bs.hg.ic.ic201.SaveGeneral", "unCheck",
		...
		private void changeVO(GeneralBillVO vo) {
			...
			int len = items.length;
			for (int i = 0; i < len; i++) {
				items[i].setAttributeValue(HgPubConst.NUM_DEF_QUA, PuPubVO
						.getUFDouble_NullAsZero(null));
			}
		}
		...
		private void setDataToUIForUnCheckDeal(GeneralBillVO billVO) {
			...
			if (BillMode.List == getM_iCurPanel()) {
				...
				setListHeadItem(head.getTs(), selrow, "ts");
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[5]),selrow,HgPubConst.VUSERDEF[5]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[6]),selrow,HgPubConst.VUSERDEF[6]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[7]),selrow,HgPubConst.VUSERDEF[7]);
				// setListHeadItem(head.getAttributeValue(HgPubConst.VUSERDEF[8]),selrow,HgPubConst.VUSERDEF[8]);
				String[] aryAssistunit = new String[] {
						"fisclose->" + HgPubConst.VUSERDEF[5],
						"updateid->" + HgPubConst.VUSERDEF[7],
						"updatename->getColValue(sm_user,user_name,cuserid,"
				...
				// setCardHeadItem(HgPubConst.VUSERDEF[5],head.getAttributeValue(HgPubConst.VUSERDEF[5]));
				// setCardHeadItem(HgPubConst.VUSERDEF[6],head.getAttributeValue(HgPubConst.VUSERDEF[6]));
				// setCardHeadItem(HgPubConst.VUSERDEF[7],head.getAttributeValue(HgPubConst.VUSERDEF[7]));
				// setCardHeadItem(HgPubConst.VUSERDEF[8],head.getAttributeValue(HgPubConst.VUSERDEF[8]));
				String[] aryAssistunit = new String[] {
						"fisclose->" + HgPubConst.VUSERDEF[5],
						"updateid->" + HgPubConst.VUSERDEF[7],
						"updatename->getColValue(sm_user,user_name,cuserid,"
		...
		private void onBillIn() {
			...
			try {
				...
				if (HgPubConst.purchaseIn_no == HgPubTool
						.getPurchaseInBusiStatus(vo)) {// 不合格
					showErrorMsg("该单据不合格,不用入库");
					return;
				}

				if (HgPubConst.purchaseIn_nopact == HgPubTool
						.getPurchaseInBusiStatus(vo)) {
					...
					UFDouble nkdnum = PuPubVO
							.getUFDouble_NullAsZero(getBillCardPanel()
									.getBillModel().getValueAt(i,
											HgPubConst.NUM_DEF_QUA));
					getBillCardPanel().getBillModel().setValueAt(nkdnum, i,
							"ninnum");
					getBillCardPanel().getBillModel().setRowState(i,
							BillModel.MODIFICATION);// 设置表体修改态
					// 表体不修改时也可以将自动带出数据保存
					items[i].setNinnum(nkdnum);
					getBillCardPanel().getBillModel().setValueAt(
							ClientEnvironment.getInstance().getDate(), i,
							"dbizdate");
					items[i].setDbizdate(ClientEnvironment.getInstance()
							.getDate());
					UFDouble hsl = PuPubVO
							.getUFDouble_ZeroAsNull(getBillCardPanel()
									.getBillModel().getValueAt(i, "hsl"));
					if (hsl != null) {
						getBillCardPanel().getBillModel().setValueAt(
								nkdnum.div(hsl, HgPubConst.NUM_DIGIT), i,
								"ninassistnum");
						items[i].setNinassistnum(nkdnum.div(hsl,
								HgPubConst.NUM_DIGIT));
					}
					UFDouble nprice = PuPubVO
							.getUFDouble_NullAsZero(getBillCardPanel()
									.getBillModel().getValueAt(i, "nprice"));

					UFDouble nplanprice = PuPubVO
							.getUFDouble_NullAsZero(getBillCardPanel()
									.getBillModel().getValueAt(i,
											"nplannedprice"));

					getBillCardPanel().getBillModel().setValueAt(
							nkdnum.multiply(nprice, HgPubConst.MNY_DIGIT), i,
							"nmny");
					getBillCardPanel().getBillModel().setValueAt(
							nkdnum.multiply(nplanprice, HgPubConst.MNY_DIGIT),
							i, "nplannedmny");

					items[i].setNmny(nkdnum.multiply(nprice,
							HgPubConst.MNY_DIGIT));
					items[i].setNplannedmny(nkdnum.multiply(nplanprice,
							HgPubConst.MNY_DIGIT));
		...
		private void setButtonEnable(String buttonName, boolean flag) {
			getButtonManager().getButtonTree().getButton(buttonName)
					.setEnabled(flag);
			getClientUI()
					.updateButton(getButtonManager().getButton(buttonName));// 设置操作按钮不可用
		}
		...
		protected void onCancel() {
			...
			if (!PuPubVO.getUFBoolean_NullAs(getBillCardPanel().getHeadItem("freplenishflag")
				...
				// 更新入库数量
				for (int i = 0; i < len; i++) {
					if (!UFDouble.ZERO_DBL.equals(PuPubVO
							.getUFDouble_NullAsZero(items[i].getNinnum()))) {
						getBillCardPanel().getBillModel().setValueAt(
								UFDouble.ZERO_DBL, i, "ninnum");
						...
						if (hsl != null) {
							getBillCardPanel().getBillModel().setValueAt(
									UFDouble.ZERO_DBL
											.div(hsl, HgPubConst.NUM_DIGIT), i,
									"ninassistnum");
							items[i].setNinassistnum(UFDouble.ZERO_DBL.div(hsl,
									HgPubConst.NUM_DIGIT));
						}
						UFDouble nprice = PuPubVO
								.getUFDouble_NullAsZero(getBillCardPanel()
										.getBillModel().getValueAt(i, "nprice"));
						getBillCardPanel().getBillModel().setValueAt(
								UFDouble.ZERO_DBL.multiply(nprice,
										HgPubConst.MNY_DIGIT), i, "nmny");
						items[i].setNmny(UFDouble.ZERO_DBL.multiply(nprice,
								HgPubConst.MNY_DIGIT));
					}
				}
				// getM_alListData().set(getM_iLastSelListHeadRow(), vo);
		...
		// private void setNumToUI(GeneralBillVO billVO) {
		// GeneralBillItemVO[] items = (GeneralBillItemVO[]) billVO
		// .getChildrenVO();
		// int len = items.length;
		// for (int i = 0; i < len; i++) {
		...
		// .getAttributeValue("nkdnum")), i, "ninnum");
		// } else {
		// getBillCardPanel().getBillModel().setValueAt(
		// PuPubVO.getUFDouble_NullAsZero(items[i]
		// .getAttributeValue("nkdnum")), i, "ninnum");
	...
	private void onMatchPact() {
		try {
			...
			if (HgPubConst.purchaseIn_in == HgPubTool
					.getPurchaseInBusiStatus(getCurVO())) {
			...
			if (HgPubConst.purchaseIn_no == HgPubTool
					.getPurchaseInBusiStatus(getCurVO())) {// 不合格
				showWarningMessage("该单据不合格,不能合同匹配");
				return;
			}

			boolean flag = getPactDlg().initData((GeneralBillVO) getCurVO());
			if (flag) {
			...
			// GeneralBillVO vo = getPactDlg().getNewGbillvo();
			// GeneralBillItemVO[] items
			// =(GeneralBillItemVO[])vo.getChildrenVO();
