	...
	public void beforeAction(Action action, AggregatedValueObject[] billvos,
		...
		if(action==Action.AUDIT){//库存签字  实扣  
			for(GeneralBillVO gbillvo:gbillvos){		
				getScmPubBO().reWriteUseFundBefore(gbillvo);		
			}			
		}
//			else if(action == Action.UNAUDIT){//库存取消签字
//			for(GeneralBillVO gbillvo:gbillvos){		
//				getScmPubBO().checkAndUseFund(gbillvo,false,true);
//			}
//		}else
		if(action == Action.SAVE){
			...
			for(GeneralBillVO gbillvo:gbillvos){		
				...
				getScmPubBO().checkAndUseFund(gbillvo,true,true);
			}	
			//zhf end
...
//		getScmPubBO().getPlanTypeFor4Y(l5xbids)
//
//		if(HgPubTool.getIFundTypeByPlanType(
//				PuPubVO.getString_TrimZeroLenAsNull(gbillvo.getItemVOs()[0].getCsourcetype()))
//				== HgPubConst.FUND_CHECK_ERRORTYPE)
//			return;
